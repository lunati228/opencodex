...
export function createIsolatedTestEnvironment(
  ...
  const roamingAppData = join(root, "AppData", "Roaming");
  mkdirSync(opencodexHome, { recursive: true });
  mkdirSync(codexHome, { recursive: true });
  // Environment.GetFolderPath(LocalApplicationData) returns an empty string on Windows when the
  // USERPROFILE-backed known folder does not physically exist. Coordinator/ACL tests must reach
  // their real production path logic without ever falling through to the developer's live
  // LocalAppData, so give each disposable profile its own physical known-folder target.
  ...
  return {
    ...
    env: {
      ...
      ...(process.platform === "win32"
        ? { LOCALAPPDATA: localAppData, APPDATA: roamingAppData }
        : {}),
  ...
  try {
    ...
    const child = Bun.spawnSync(
      [
        ...
        "test",
        "--isolate",
        // Bun's 5 s default is a general-purpose number, not a gate anyone chose
        // for this suite, and it is too tight here: many tests spawn the real
        // CLI through process.execPath, which costs ~1.3 s per spawn on Windows
        // (cli-provider.test.ts: 26 tests, 33.76 s unloaded). That leaves so
        // little headroom that under the full 381-file matrix they cross the
        // line -- 12 failures, all at ~5,015-5,430 ms, none an assertion.
        //
        // Raising it changes no assertion and skips nothing; it only stops
        // timing out work that is legitimately slow. Measured: 4941 pass /
        // 13 fail at 5 s, 4953 pass / 1 fail at 30 s, same 351 s wall clock.
        // 30 s still leaves 2.2x over the slowest honest test (13.4 s), so a
        // genuine hang is still caught.
        //
        // Placed before the caller's arguments so an explicit --timeout wins.
        "--timeout",
        "30000",
