...
推論層の可視性は、v1/base/v2 サーフェス モードから独立しています。明示された非 GPT ラダーは、合成の `max` や `ultra` を追加せず正確に表示されます。Gemini 3.8 Flash は `low` / `medium` / `high`、管理対象のローカル Qwen は `low` / `medium` / `xhigh` のみです。GPT の動作は変更されず、上流の GPT-5.6 ラダーも正確に保存されます。
