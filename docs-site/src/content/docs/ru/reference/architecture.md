...
`reasoning-effort.ts` преобразует уровни рассуждения Codex в значения протокола провайдера. Исходящий запрос использует поддерживаемую провайдером лестницу; обычные маршрутизируемые модели также могут показывать синтетические продуктовые уровни. Модуль:
...
Обычные маршрутизируемые модели сохраняют синтетические уровни `max` и `ultra` в списке выбора.
Для Gemini 3.8 Flash оба значения преобразуются в поддерживаемый провайдером уровень `high`.
Управляемый локальный Qwen сохраняет только `low` / `medium` / `xhigh`, по умолчанию — `xhigh`.
