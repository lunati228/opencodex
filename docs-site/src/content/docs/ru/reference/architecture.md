...
Явно объявленная лестница не-GPT публикуется без синтетических ступеней. Записи без объявленной
лестницы сохраняют совместимые значения по умолчанию, а модели GPT — существующие продуктовые
ступени Codex. Модуль:
...
Например, Gemini 3.7 Flash показывает только `low` / `medium` / `high`, а управляемый локальный
Qwen — `low` / `medium` / `xhigh` с `xhigh` по умолчанию. Старые значения ограничиваются или
сопоставляются только на wire-границе.
