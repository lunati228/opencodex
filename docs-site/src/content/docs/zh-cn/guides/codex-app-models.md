...
推理档位的可见性与 v1/base/v2 界面模式无关。已声明的非 GPT 阶梯会原样显示，不会加入合成的 `max` 或 `ultra`；例如 Gemini 3.7 Flash 只有 `low` / `medium` / `high`，托管 Qwen 只有 `low` / `medium` / `xhigh`。GPT 行为保持不变，精确的 GPT-5.6 阶梯也会原样保留。
