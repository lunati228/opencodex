...
## 2026-07-29 integration surfaces
...
- `PUT /api/helper-turn-models` accepts review target, compact target,
  ...
  0..100 and `null` clears it. The route validates the complete body before
  mutating config, so invalid later fields cannot partially save.
- Provider-quota refresh and MCP usage refresh perform metadata-only catalog
  decoration. They can change quota text but cannot add, remove, or reorder
  catalog entries.
- The Codex picker contains two exact non-routable readouts: `AGY Usage` for the
  OpenCodex login and `MCP Usage` for the separate CLI/MCP identity.
- NVIDIA provider cards and picker rows use the same friendly display map while
  routing continues to use stable internal provider/model ids.
