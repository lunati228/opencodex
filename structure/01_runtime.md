...
## Codex-coupled companion lifecycle

This fork's required desktop lifecycle uses `ocx codex-companion run`, launched
at user login through `OpenCodex Codex Companion.lnk` and a hidden VBS launcher.
The lightweight watcher observes `codex.exe`; it starts the Bun proxy when
Codex appears, releases the owned local model after Codex closes, then stops
the proxy on a later pass. Ownership prevents it from stopping a proxy it did
not start.

This differs from `ocx service`: the service is intentionally always-on between
login and logout, while the user's requirement is Codex-coupled proxy lifetime.
Generic startup-health currently reports service/shim protection and does not
classify companion mode. Companion acceptance therefore comes from its status
command and recorded start/close/release/stop edges.
