...
describe("headless GUI parity CLI", () => {
  test("every GUI management endpoint belongs to a documented CLI resource", () => {
    ...
    const coverage: Array<[string, string]> = [
      ...
      ["/api/keys", "ocx access"],
      // Fork-added. The managed local engine costs ~28 GB and both GPUs, so a headless
      // operator needs to see, size and stop it; `ocx local-runtime` is that surface.
      ["/api/local-runtime", "ocx local-runtime"],
      ["/api/agy-usage", "ocx observe agy-usage"],
      // Fork-added and not yet referenced from gui/src, so this gate would not catch them
  ...
  test("Responses access test sends the native-compatible streamed request shape", async () => {
    const runtime = fakeRuntime((req) => new URL(req.url).pathname === "/v1/responses"
      ? new Response(
    ...
    expect(await handleAccessCommand([
      "test", "gpt-5.6-sol", "--protocol", "responses", "--json",
    ], runtime.deps)).toBe(0);
    expect(runtime.requests).toEqual([{
      path: "/v1/responses",
      method: "POST",
  ...
  test("Responses access test rejects a failed terminal without echoing its payload", async () => {
    const marker = "private-upstream-detail";
    const runtime = fakeRuntime((req) => new URL(req.url).pathname === "/v1/responses"
      ? new Response(
        `event: response.failed\ndata: ${JSON.stringify({
          type: "response.failed",
          response: { status: "failed", error: { message: marker } },
        })}\n\n`,
        { headers: { "Content-Type": "text/event-stream" } },
      )
      : undefined);
