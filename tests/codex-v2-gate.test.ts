...
describe("catalog ultra (always-on)", () => {
  ...
  test("native and routed ladders universally expose synthetic max + ultra", () => {
    // OpenCodex validates spawn_agent effort overrides against catalog membership. Upstream
    // therefore appends synthetic max and ultra even when a provider declares its lower
    // rungs; provider-specific request mapping clamps those synthetic values on the wire.
    ...
    // Native path is untouched: gpt-5.5's real ladder stops at xhigh and the wire clamp
    // repairs max/ultra, so advertising them costs nothing and keeps spawns validating.
    expect(efforts(native)).toContain("ultra");
    expect(efforts(native)).toContain("max");
    expect(efforts(glm)).toEqual(["low", "medium", "high", "xhigh", "max", "ultra"]);
  });

  test("a routed model with NO declared ladder still gets mock max + ultra", () => {
    // Unknown ladders stay permissive for the same spawn_agent validation contract.
    const undeclared = [{ id: "mystery-1", provider: "opencode-go" }];
    const entries = buildCatalogEntries(template(), [], undeclared as never, [], false);
    const row = entries.find(e => e.slug === "opencode-go/mystery-1")!;
    expect(efforts(row)).toContain("max");
    expect(efforts(row)).toContain("ultra");
  });
