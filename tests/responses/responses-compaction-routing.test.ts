...
import { clearProviderQuotaCache } from "../../src/providers/quota";
import * as providerQuotaModule from "../../src/providers/quota";
...
describe("native compact usage reporting", () => {
  ...
  test("main-pool and legacy added accounts carry their effective usage labels", async () => {
    ...
    try {
      ...
      expect((await handleResponsesCompact(compactionRequest(baseCompactionBody({ model: "gpt-5.5" })), mainConfig, mainLog)).status).toBe(200);
      ...
      expect((await handleResponsesCompact(compactionRequest(baseCompactionBody({ model: "gpt-5.5" })), poolConfig, poolLog)).status).toBe(200);
...
describe("routed compaction for key-mode openai-responses (#422)", () => {
  test("a stopped local source compacts through the external helper without loading itself", async () => {
    clearProviderQuotaCache();
    const urls: string[] = [];
    globalThis.fetch = (async (url: unknown) => {
      urls.push(String(url));
      return jsonResponse(completedPayload("stopped local handoff"));
    }) as typeof fetch;
    const config = keyProviderConfig();
    config.providers[QWEN_PROFILE.providerId] = managedLocalProviderProjection(
      QWEN_PROFILE.defaultContext,
      QWEN_PROFILE.id,
    );
    config.autoCompactModel = "gw/some-model";
    config.helperTurnScope = "external";
    config.helperTurnReasoningEffort = "high";
    config.helperTurnCodexRemainingPercentThreshold = 5;
    const logCtx: RequestLogContext = { model: "", provider: "" };

    const response = await handleResponsesCompact(
      compactionRequest({
        model: `${QWEN_PROFILE.providerId}/${QWEN_PROFILE.modelId}`,
        input: [
          {
            type: "message",
            role: "user",
            content: [{ type: "input_text", text: "preserve this harmless state" }],
          },
        ],
    ...
    expect(urls[0]).toContain("gateway.example");
    expect(logCtx.helperTurn).toBe("auto-compact");
    expect(logCtx.helperSourceModel).toBe(`${QWEN_PROFILE.providerId}/${QWEN_PROFILE.modelId}`);
    expect(logCtx.helperReasoningEffort).toBe("high");
    const json = await response.json() as { output?: unknown[] };
    expect(JSON.stringify(json.output)).toContain("stopped local handoff");
  });

  test("an unknown quota still sends external compaction to the configured helper", async () => {
    clearProviderQuotaCache();
    ...
    globalThis.fetch = (async (url: unknown, init?: RequestInit) => {
      ...
      return jsonResponse(completedPayload("native external-model summary"));
    }) as typeof fetch;
    const config = keyProviderConfig();
    config.autoCompactModel = "gw/some-model";
    config.helperTurnScope = "external";
  ...
  test("one compaction request reads one frozen helper quota snapshot", async () => {
    const quotaRead = spyOn(providerQuotaModule, "getCachedProviderQuotaReports").mockReturnValue(null);
    try {
      globalThis.fetch = (async () => jsonResponse(completedPayload("summary"))) as typeof fetch;
      const response = await handleResponses(
        compactionRequest(baseCompactionBody()),
        keyProviderConfig(),
        { model: "", provider: "" },
      ...
      quotaRead.mockRestore();
    }
  });

  test("an active same-model compaction override still gets helper logging and high effort", async () => {
    const bodies: Array<Record<string, unknown>> = [];
    ...
    expect(response.status).toBe(200);
    expect(logCtx.helperTurn).toBe("auto-compact");
    expect(logCtx.helperSourceModel).toBe("gw/some-model");
    expect(logCtx.helperReasoningEffort).toBe("high");
    expect(bodies[0]?.reasoning).toEqual({ effort: "high" });
  });

  test("external compaction stays on the helper on both sides of the quota boundary", async () => {
    const now = Date.now();
    let usedPercent = 94.999;
    const quotaRead = spyOn(providerQuotaModule, "getCachedProviderQuotaReports").mockImplementation(() => ({
      generatedAt: now,
      reports: [{
        provider: "openai",
        label: "OpenAI Codex",
        source: "test",
        quota: { weeklyPercent: usedPercent, updatedAt: now },
        updatedAt: now,
      }],
    }));
    try {
      ...
      for (const used of [94.999, 95]) {
        ...
        expect(logCtx.helperTurn).toBe("auto-compact");
        expect(logCtx.helperReasoningEffort).toBe("high");
        expect(bodies.at(-1)?.reasoning).toEqual({ effort: "high" });
      }
    } finally {
      quotaRead.mockRestore();
    }
...
describe("compact alternate-account attempt (#913)", () => {
  ...
  test("a bound thread at 100% local quota still sends once, with no alternate attempt", async () => {
    ...
    await withPoolEnv("ocx-compact-quota-100-", async config => {
      ...
      const res = await handleResponsesCompact(
        compactionRequest(baseCompactionBody({ model: "gpt-5.5" }), undefined, {
  ...
  test("an abort between attempts prevents the alternate send", async () => {
    await withPoolEnv("ocx-compact-alt-abort-", async config => {
      ...
      await handleResponsesCompact(
        compactionRequest(baseCompactionBody({ model: "gpt-5.5" }), abort.signal),
        config,
        { model: "", provider: "" },
      );
