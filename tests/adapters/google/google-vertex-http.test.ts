...
describe("vertex retry fetch", () => {
  test("Antigravity rejects redirects without changing Vertex redirect behavior", async () => {
    const antigravity = mockFetch([new Response("ok", { status: 200 })]);
    expect((await fetchAntigravityWithRetry(request, { timeoutMs: 5_000 })).status).toBe(200);
    expect(antigravity.calls[0]?.redirect).toBe("error");

    const vertex = mockFetch([new Response("ok", { status: 200 })]);
    expect((await fetchVertexWithRetry(request, { timeoutMs: 5_000 })).status).toBe(200);
    expect(vertex.calls[0]?.redirect).toBeUndefined();
  });
