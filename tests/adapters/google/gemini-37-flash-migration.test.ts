...
describe("3.7 reasoning control", () => {
  ...
  test.each(["low", "medium", "high"])(
    "saved 3.7 -%s ids use the tiered wire and preserve their level",
    level => {
      expect(resolveAntigravityEffortWireModel(`gemini-3.7-flash-${level}`)).toEqual({
  ...
  test("an explicit effort overrides a saved 3.7 suffix", () => {
    expect(resolveAntigravityEffortWireModel("gemini-3.7-flash-low", "high")).toEqual({
      wireModelId: "gemini-3.7-flash-tiered",
      thinkingLevel: "high",
    });
  });

  test("current suffix ids are not mislabeled as retired generations", () => {
    expect(retiredAntigravityFlashTier("gemini-3.7-flash-high")).toBeUndefined();
  });
});
