...
describe("google provider hardening", () => {
  ...
  for (const baseUrl of [
    ...
    "https://daily-cloudcode-pa.googleapis.com.attacker.example",
    "https://daily-cloudcode-pa.googleapis.com" + "@attacker.example",
    "https://user:pass" + "@daily-cloudcode-pa.googleapis.com",
    "https://daily-cloudcode-pa.googleapis.com/proxy",
    "https://daily-cloudcode-pa.googleapis.com?target=attacker",
    "https://daily-cloudcode-pa.googleapis.com#attacker",
    "https://daily-cloudcode-pa.googleapis.com:444",
  ]) {
    for (const stream of [false, true]) {
      test(`Antigravity rejects non-canonical ${stream ? "streaming" : "non-streaming"} baseUrl ${baseUrl}`, async () => {
        const adapter = createGoogleAdapter(antigravityProvider({ baseUrl }));

        await expect(adapter.buildRequest(parsed(stream))).rejects.toThrow(
          /canonical Google endpoint/i,
        );
      });
    }
  }

  test("Antigravity canonical trailing slash still builds fixed streaming and non-streaming endpoints", async () => {
    const adapter = createGoogleAdapter(antigravityProvider({
      baseUrl: "https://daily-cloudcode-pa.googleapis.com/",
    }));

    const nonStreaming = await adapter.buildRequest(parsed(false));
    const streaming = await adapter.buildRequest(parsed(true));

    expect(nonStreaming.url).toBe("https://daily-cloudcode-pa.googleapis.com/v1internal:generateContent");
    expect(streaming.url).toBe("https://daily-cloudcode-pa.googleapis.com/v1internal:streamGenerateContent?alt=sse");
    expect(nonStreaming.headers.Authorization).toBe("Bearer antigravity-test-token");
    expect(JSON.parse(nonStreaming.body).project).toBe("project-test");
  });
  ...
  test("sends Gemini Flash thinkingLevel through each transport's owned request path", async () => {
    ...
    // Antigravity owns a separate CCA envelope path. Saved 3.7 suffix ids now carry their
    // encoded tier through thinkingLevel on the single current `-tiered` wire id.
