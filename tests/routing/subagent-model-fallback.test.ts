...
test("the native-main drain sentinel covers the flagships without widening to gpt-5.5", () => {
  ...
  test("noteSubagentModelFailure records a provider model end-of-life response as a health block", () => {
    resetSubagentModelFallbackStateForTests();
    noteSubagentModelFailure(
      "kimi/k3",
      "Provider error 410: The model 'kimi/k3' has reached its end of life and is no longer available.",
      cfg(),
    );
    expect(isSubagentModelUnavailable("kimi/k3", cfg())).toBe(true);
  });
  ...
  test("noteSubagentModelFailure ignores unrelated errors", () => {
    ...
    noteSubagentModelFailure(
      "kimi/k3",
      "invalid_request_error: model parameter 'tools' is unsupported for this request shape",
      cfg(),
    );
    expect(isSubagentModelUnavailable("kimi/k3", cfg())).toBe(false);
  });
