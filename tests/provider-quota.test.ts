...
describe("fetchProviderQuotaReports", () => {
  ...
  for (const baseUrl of [
    ...
    test(`Antigravity quota rejects non-canonical boundary ${baseUrl}`, async () => {
      await saveCredential("google-antigravity", {
        access: "agy-access-secret",
        refresh: "agy-refresh-secret",
        expires: Date.now() + 3600_000,
      ...
      config.providers["google-antigravity"]!.baseUrl = baseUrl;
      const result = await fetchProviderQuotaReports(config, true);

      expect(result.reports.find(row => row.provider === "google-antigravity")).toBeUndefined();
      expect(seen.some(url => url.includes("cloudcode-pa.googleapis.com"))).toBe(false);
    });
  }

  test("Antigravity quota accepts a trailing slash but sends only to the fixed canonical endpoint", async () => {
    await saveCredential("google-antigravity", {
      access: "agy-access-secret",
      refresh: "agy-refresh-secret",
      expires: Date.now() + 3600_000,
    ...
    const seen: Array<{ url: string; authorization?: string; body?: string; redirect?: RequestRedirect }> = [];
    globalThis.fetch = (async (input: RequestInfo | URL, init?: RequestInit) => {
      const headers = init?.headers as Record<string, string> | undefined;
      seen.push({
        url: String(input),
    ...
    const config = testConfig();
    config.providers["google-antigravity"]!.baseUrl = "https://daily-cloudcode-pa.googleapis.com/";
    const result = await fetchProviderQuotaReports(config, true);

    expect(result.reports.find(row => row.provider === "google-antigravity")).toBeDefined();
    expect(seen.filter(row => row.url.includes("cloudcode-pa.googleapis.com"))).toEqual([{
