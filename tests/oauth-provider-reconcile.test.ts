...
describe("OAuth provider reconciliation", () => {
  ...
  test("does not promote a marked noncanonical Antigravity static row", () => {
    const preset = structuredClone(OAUTH_PROVIDERS["google-antigravity"].providerConfig);
    const nonLegacyFlashModel = "gemini-3.8-flash";
    const config = {
      port: 10100,
      defaultProvider: "google-antigravity",
      googleAntigravityStaticCatalogVersion: 1,
      providers: {
        "google-antigravity": {
          ...preset,
          defaultModel: nonLegacyFlashModel,
          models: [nonLegacyFlashModel, ...preset.models!.slice(1)],
          liveModels: false,
        },
      },
    } satisfies OcxConfig;
