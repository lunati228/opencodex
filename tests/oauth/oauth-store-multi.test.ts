...
describe("multi-account auth store", () => {
  ...
  test("Antigravity credential persistence gets one bounded recovery after an ACL timeout", async () => {
    setPlatformForTests("win32");
    ...
    try {
      await expect(saveCredential("google-antigravity", cred({
        accountId: "antigravity-reviewer",
      ...
      expect(getCredential("google-antigravity")?.access).toBe("recovered-access");
  ...
  test("unchanged selection admission neither joins a busy writer nor persists", async () => {
    ...
    const blocker = mutateStore(async () => { entered(); await gate; });
    await started;
    const write = spyOn(atomicWrite, "atomicWriteFileAsync");
  ...
  test("selection events follow persistence and omit failed commits, refreshes, and credentials", async () => {
    ...
    try {
      ...
      const write = spyOn(atomicWrite, "atomicWriteFileAsync").mockImplementation(() => { throw new Error("selection persist failed"); });
      try {
