...
describe("Google Gemini catalog metadata", () => {
  test("preserves direct Google Gemini 3.6 Flash's declared reasoning ladder", async () => {
    const google = {
      adapter: "google",
    ...
    // The registry ladder is authoritative for non-GPT models; picker-only synthetic
    // agent rungs must not claim provider support.
...
describe("routed reasoning picker fidelity", () => {
  test("keeps Gemini exact while leaving routed GPT synthesis unchanged", () => {
    ...
    const efforts = (slug: string) => {
      const levels = (
        entries.find(entry => entry.slug === slug)?.supported_reasoning_levels
      ) as Array<{ effort: string }> | undefined;
      return (levels ?? []).map(level => level.effort);
    };

    expect(efforts("google-antigravity/gemini-3.7-flash"))
      .toEqual(["low", "medium", "high"]);
    expect(efforts("example/gpt-5.6-sol"))
      .toEqual(["low", "medium", "high", "xhigh", "max", "ultra"]);
  });

  test("repairs stale preserved non-GPT ladders without changing preserved GPT rows", () => {
    ...
    const merged = mergeObservedForTest({
      catalogModels: [
        ...
        {
          ...nativeTemplate(),
          slug: "example/gpt-5.6-sol",
          supported_reasoning_levels: levels,
          default_reasoning_level: "max",
        },
      ],
      routedEntries: [],
      gatheredProviderNames: new Set(["google-antigravity", "example"]),
      degradedProviderNames: new Set(["google-antigravity", "example"]),
    });
    const efforts = (slug: string) => {
      const supported = merged.find(entry => entry.slug === slug)
        ?.supported_reasoning_levels as Array<{ effort: string }> | undefined;
      return supported?.map(level => level.effort);
    };

    expect(efforts("google-antigravity/gemini-3.7-flash"))
      .toEqual(["low", "medium", "high"]);
    expect(merged.find(entry => entry.slug === "google-antigravity/gemini-3.7-flash")
      ?.default_reasoning_level).toBe("medium");
    expect(efforts("example/gpt-5.6-sol"))
      .toEqual(["low", "medium", "high", "xhigh", "max", "ultra"]);
  });
});
...
describe("configured CatalogModel displayName -> catalog display_name", () => {
  ...
  test("a customModel reasoning ladder overrides the inherited provider ladder end-to-end", async () => {
    ...
    try {
      ...
      // Non-GPT declarations are exact: inherited levels and synthetic product tiers stay out.
      expect(levels).toEqual(["medium", "max"]);
  ...
  test("a mixed none+low custom ladder remains exact and keeps none first", async () => {
    clearModelCache("custom-provider");
    const originalFetch = globalThis.fetch;
    globalThis.fetch = (() => {
      throw new Error("fetch should not be called");
    ...
    try {
      ...
      const row = entries.find(e => e.slug === "custom-provider/renamed-model");
      const levels = (row?.supported_reasoning_levels ?? []).map((l: { effort: string }) => l.effort);
      expect(levels).toEqual(["none", "low"]);
...
describe("Codex catalog routed normalization", () => {
  ...
  test("routed entries fall to the conservative triple instead of inheriting template context (#992)", () => {
    ...
    expect(routed?.auto_compact_token_limit).toBe(autoCompactLimitFor(128_000));
  });
  ...
  test("native gpt-5.4 uses its 1M context window override", () => {
    ...
    expect(native?.auto_compact_token_limit).toBe(autoCompactLimitFor(1_000_000));
  });
  ...
  test("native gpt-5.3-codex-spark uses its 100k context window instead of inherited codex max", () => {
    ...
    expect(native?.auto_compact_token_limit).toBe(autoCompactLimitFor(100_000));
  });
  ...
  test("native GPT-5.6 entries add max and ultra reasoning even when cloned from an older template", () => {
    ...
    expect(gpt56?.auto_compact_token_limit).toBe(autoCompactLimitFor(272_000));
  ...
  test("providerContextCaps.openai ceilings native GPT-5.6 catalog rows (#1430)", () => {
    ...
    for (const slug of ["gpt-5.6-sol", "gpt-5.6-terra", "gpt-5.6-luna"]) {
      ...
      expect(entry?.auto_compact_token_limit).toBe(autoCompactLimitFor(cap));
    }
  });
  ...
  test("mergeCatalogEntriesForSync re-applies the openai cap to preserved and upgraded native rows (#1430)", () => {
    ...
    expect(sol?.auto_compact_token_limit).toBe(autoCompactLimitFor(cap));
    ...
    expect(luna?.auto_compact_token_limit).toBe(autoCompactLimitFor(cap));
  });
  ...
  test("preserved gpt-5.4-mini rows get the openai cap without a hardcoded override (#1430)", () => {
    ...
    expect(mini?.auto_compact_token_limit).toBe(autoCompactLimitFor(cap));
  });
  ...
  test("Daybreak Blue inherits Sol capabilities and ships one bare row plus one row per selector", () => {
    ...
    expect(daybreak?.auto_compact_token_limit).toBe(autoCompactLimitFor(272_000));
  ...
  test("configured ChatGPT-forward Daybreak gets Sol native metadata without API-key crossover", async () => {
    ...
    expect(daybreak).toMatchObject({
      ...
      context_window: 922_000,
      max_context_window: 922_000,
      auto_compact_token_limit: autoCompactLimitFor(922_000),
      comp_hash: "3000",
      tool_mode: "code_mode_only",
      use_responses_lite: true,
      supports_parallel_tool_calls: true,
  ...
  test("routed entries receive exact jawcode context metadata", () => {
    ...
    expect(routed?.auto_compact_token_limit).toBe(autoCompactLimitFor(1_000_000));
  ...
  test("DeepSeek routed entries restore the official context window from jawcode metadata", () => {
    ...
    for (const routed of [flash, pro]) {
      ...
      expect(routed?.auto_compact_token_limit).toBe(autoCompactLimitFor(1_048_576));
      expect(routed?.input_modalities).toEqual(["text"]);
    }
  ...
  test("provider context-cap applies before jawcode catalog metadata reaches Codex", () => {
    ...
    expect(routed?.auto_compact_token_limit).toBe(autoCompactLimitFor(350_000));
    expect(routed?.input_modalities).toEqual(["text"]);
  ...
  test("opencode-go high-risk models use official jawcode metadata in the Codex catalog", () => {
    const cases = [
      { id: "glm-5.2", context: 1_000_000, auto: autoCompactLimitFor(1_000_000), input: ["text"] },
      ...
      { id: "qwen3.5-plus", context: 1_000_000, auto: autoCompactLimitFor(1_000_000), input: ["text", "image"] },
      { id: "kimi-k2.7-code", context: 262_144, auto: autoCompactLimitFor(262_144), input: ["text", "image"] },
      { id: "minimax-m3", context: 512_000, auto: autoCompactLimitFor(512_000), input: ["text", "image"] },
      { id: "qwen3.7-max", context: 1_000_000, auto: autoCompactLimitFor(1_000_000), input: ["text"] },
    ] as const;
  ...
  test("routed entries resolve jawcode provider aliases", () => {
    ...
    expect(routed?.auto_compact_token_limit).toBe(autoCompactLimitFor(262_144));
    expect(routed?.input_modalities).toEqual(["text", "image"]);
  });
  ...
  test("known routed metadata still restores the exact context window (#992)", () => {
    ...
    expect(routed?.auto_compact_token_limit).toBe(autoCompactLimitFor(256_000));
  });
  ...
  test("built-in DeepSeek and GLM effort models opt into Codex reasoning propagation (#1100)", async () => {
    const expected = [
      { slug: "deepseek/deepseek-v4-flash", efforts: ["low", "high", "max"] },
      { slug: "deepseek/deepseek-v4-pro", efforts: ["low", "high", "max"] },
      ...
      { slug: "zai/glm-5.2", efforts: ["low", "medium", "high", "xhigh", "max"] },
      { slug: "zai/glm-5.2[1m]", efforts: ["low", "medium", "high", "xhigh", "max"] },
      { slug: "zhipu-bigmodel/glm-4.6", efforts: ["low", "medium", "high", "xhigh", "max"] },
      { slug: "zhipu-bigmodel/glm-4.7", efforts: ["low", "medium", "high", "xhigh", "max"] },
      { slug: "zhipu-bigmodel/glm-5", efforts: ["low", "medium", "high", "xhigh", "max"] },
      { slug: "zhipu-bigmodel/glm-5.1", efforts: ["low", "medium", "high", "xhigh", "max"] },
    ];
    const models = await gatherRoutedModels({
      providers: {
        deepseek: {
  ...
  test("provider config model metadata reaches Codex catalog for static models", async () => {
    ...
    expect(routed?.auto_compact_token_limit).toBe(autoCompactLimitFor(321_000));
    expect(routed?.input_modalities).toEqual(["text", "image"]);
  });
  ...
  // min(autoCompactLimitFor(contextWindow), maxInputTokens), so setting one would move the expectation.
...
describe("OpenAI API trusted catalog augmentation", () => {
  ...
  test("routed auto-compaction is bounded by max-input after effective context caps", () => {
    ...
    expect(entries.find(row => row.slug === "openai-apikey/gpt-5.6-sol")?.auto_compact_token_limit)
      .toBe(Math.min(autoCompactLimitFor(1_050_000), 922_000));
    expect(entries.find(row => row.slug === "openai-apikey/gpt-5.6-terra")?.auto_compact_token_limit)
      .toBe(Math.min(autoCompactLimitFor(350_000), 922_000));
  });
