...
describe("antigravity CCA envelope", () => {
  ...
  test("preserves Codex json_schema output on Gemini reviewer requests", async () => {
    const reviewer = parsed("review this command", false, "gemini-3.7-flash");
    const schema = {
      type: "object",
      properties: {
    ...
    const req = await createGoogleAdapter(provider).buildRequest(reviewer);
    const env = JSON.parse(req.body);
    expect(env.request.generationConfig?.responseMimeType).toBe("application/json");
    expect(env.request.generationConfig?.responseSchema).toEqual(schema);
    expect(env.request.generationConfig?.responseFormat).toBeUndefined();
  });
  ...
  test("ignores inherited CCA model, alias, and usage-map properties", () => {
    ...
    expect(resolveAntigravityEffortWireModel("gemini-3.1-pro", "__proto__")).toEqual({
      wireModelId: "gemini-pro-agent",
    });
    expect(resolveAntigravityEffortWireModel("gemini-3.1-pro", "constructor")).toEqual({
      wireModelId: "gemini-pro-agent",
    });
    expect(canonicalAntigravityUsageModel("__proto__")).toBe("__proto__");
    expect(canonicalAntigravityUsageModel("constructor")).toBe("constructor");
  });
