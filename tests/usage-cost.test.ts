...
describe("resolveMatchedPrice", () => {
  ...
  test("16. shipped overlay membership: 56 keys, including Opus 5 and compatibility prices", () => {
    ...
    for (const expected of [
      ...
      "openai-apikey/gpt-5.6-terra-pro",
      "openai-apikey/gpt-5.6-luna-pro",
    ...
    const antigravity = findExpectedPriceOverlay("google-antigravity", "gemini-3.7-flash");
    expect(antigravity).toMatchObject({
      cost4: { input: 0.75, output: 3.75, cacheRead: 0.075, cacheWrite: 0 },
      status: "verified-derived",
    });
    expect(antigravity?.source).toContain("promotional rate through 2026-12-31");
    expect(antigravity?.source).toContain("ai.google.dev/gemini-api/docs/pricing");
    const antigravity36 = findExpectedPriceOverlay("google-antigravity", "gemini-3.6-flash");
    expect(antigravity36).toMatchObject({
      cost4: { input: 1.5, output: 7.5, cacheRead: 0.15, cacheWrite: 0 },
      status: "verified",
    });
    for (const [modelId, target] of [
      ...
      expect(compatibility?.source).toContain(target);
    }
