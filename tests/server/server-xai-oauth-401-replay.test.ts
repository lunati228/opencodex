...
describe("xAI OAuth Responses opt-in upstream 401 replay", () => {
  test("initial OAuth refresh projects raw provider failures before responding", async () => {
    ...
    try {
      ...
      // The fork primes quota decoration at startup, so an already-expired xAI credential has
      // one best-effort quota refresh followed by the inference request's own refresh attempt.
      expect(observed.counts.refresh).toBe(2);
