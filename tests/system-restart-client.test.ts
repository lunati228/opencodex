...
describe("bound system restart client", () => {
  ...
  test("posts once with a process-scoped capability and no reusable admin token", async () => {
    ...
    expect(restartHeaders.has(SYSTEM_RESTART_REQUIRE_IDLE_HEADER)).toBe(false);
  ...
  test("sends the companion idle guard only when explicitly requested", async () => {
    const setup = successfulDeps();
    const outcome = await requestBoundSystemRestart(target, 10_000, {
      ...setup.deps,
      requireIdle: true,
    ...
    expect(outcome).toEqual({ accepted: true });
    const headers = new Headers(setup.requests[1]!.init?.headers);
    expect(headers.get(SYSTEM_RESTART_REQUIRE_IDLE_HEADER)).toBe("1");
  });
