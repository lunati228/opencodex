...
describe("compaction is never installed from a truncated turn", () => {
  ...
  test("no compaction item when the adapter emitted no terminal", () => {
    const json = buildResponseJSON(
      [{ type: "text_delta", text: "half a summary" }],
      "routed/model",
      { compaction: true },
    );

    // Fork policy is stricter for compaction than for an ordinary buffered EOF: replacement
    // history requires an explicit clean terminal, so this is an invalid compaction response.
    expect(json.status).toBe("failed");
    expect((json.error as { message?: string } | undefined)?.message).toContain("without a terminal done event");
  ...
  test("compaction still emitted for a genuinely completed turn", () => {
    const json = buildResponseJSON(
      [{ type: "text_delta", text: "a whole summary" }, { type: "done" }],
      "routed/model",
      { compaction: true },
    );
