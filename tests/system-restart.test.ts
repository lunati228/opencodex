...
describe("POST /api/system/restart", () => {
  ...
  test("companion idle guard refuses busy proxy before scheduling drain", async () => {
    let scheduled = 0;
    setSystemRestartIoForTests({
      isDraining: () => false,
      getActiveTurnCount: () => 2,
    ...
    const req = new Request("http://127.0.0.1:10100/api/system/restart", {
      method: "POST",
      headers: { [SYSTEM_RESTART_REQUIRE_IDLE_HEADER]: "1" },
    });
    const res = await handleManagementAPI(req, new URL(req.url), config());
    expect(res?.status).toBe(423);
    expect(scheduled).toBe(0);
    expect(await res?.json()).toMatchObject({ success: false, activeTurnCount: 2 });
  });

  test("companion idle guard accepts and schedules when no turn is active", async () => {
    let scheduled = 0;
    setSystemRestartIoForTests({
      isDraining: () => false,
      getActiveTurnCount: () => 0,
