...
describe("ocx v2 keep-native-v1", () => {
  ...
  test.each(["v1", "v2"] as const)("an explicit global V2 enable cannot override saved %s pins", async mode => {
    isolateHomes();
    saveConfig({ ...loadConfig(), multiAgentMode: mode, keepNativeChatGptOnV1: true });
    const codexConfig = join(process.env.CODEX_HOME!, "config.toml");
    writeFileSync(codexConfig, "[features.multi_agent_v2]\nenabled = false\n");
    ...
    expect(isMultiAgentV2Enabled(codexConfig)).toBe(false);
    expect(errors.join("\n")).toContain("global multi_agent_v2 overrides catalog pins");
  });
...
test("/api/v2 refuses a flag-only override of the saved V1 mode", async () => {
  isolateHomes();
  const config = { ...loadConfig(), multiAgentMode: "v1" as const };
  saveConfig(config);
  let toggles = 0;
  const response = await handleManagementAPI(putV2({ enabled: true }), new URL("http://localhost/api/v2"), config, {
