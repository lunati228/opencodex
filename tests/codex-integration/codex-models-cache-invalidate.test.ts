...
describe("invalidateCodexModelsCache write gate (#476 / #518)", () => {
  ...
  test("repairs retired rows and Qwen labels when cache sync starts from a persisted legacy catalog", () => {
    const qwen128k = qwenContextVariantModelId(QWEN_PROFILE.modelId, 131_072);
    const legacyModels = [
      { slug: "gpt-5.5", display_name: "GPT 5.5" },
      { slug: AGY_PROVIDER_USAGE_ROW_SLUG, display_name: "AGY Usage | Not read" },
      { slug: AGY_BRIDGE_ROW_SLUG, display_name: "MCP Usage | not read" },
      {
        slug: `${QWEN_PROFILE.providerId}/${qwen128k}`,
        display_name: "qwen-local/huihui-qwen3.8-27b-abliterated-q6-k-l · 128K",
      },
      {
        slug: `${QWEN_PROFILE.providerId}/${QWEN_PROFILE.modelId}`,
        display_name: "Huihui-Qwen3.8-27B-abliterated-Q6_K_L",
      },
    ];
    writeFileSync(join(codexHome, "opencodex-catalog.json"), JSON.stringify({ models: legacyModels }, null, 2) + "\n");
    writeFileSync(join(codexHome, "models_cache.json"), JSON.stringify({ models: legacyModels }, null, 2) + "\n");
    ...
    const slugs = cache.models.map(model => model.slug);
    expect(slugs).not.toContain(AGY_PROVIDER_USAGE_ROW_SLUG);
    expect(slugs).not.toContain(AGY_BRIDGE_ROW_SLUG);
  ...
  test("saved V1 cannot be undone by a cache refresh from an older V2 catalog", () => {
    const slugs = ["gpt-6-astra", "google-antigravity/gemini-3.8-flash", `${QWEN_PROFILE.providerId}/${QWEN_PROFILE.modelId}`];
    writeFileSync(join(opencodexHome, "config.json"), JSON.stringify({
      ...emptyConfig, multiAgentMode: "v1", keepNativeChatGptOnV1: true,
    }));
    writeFileSync(join(codexHome, "opencodex-catalog.json"), JSON.stringify({
      models: slugs.map(slug => ({ slug, multi_agent_version: "v2" })),
    }));
    writeFileSync(join(codexHome, "models_cache.json"), JSON.stringify({
      models: [{ slug: "gpt-6-astra", multi_agent_version: "v2" }],
    }));
    expect(invalidateCodexModelsCache()).toBe(true);
    const cache = JSON.parse(readFileSync(join(codexHome, "models_cache.json"), "utf8"));
    expect(cache.models.map((row: { slug: string }) => row.slug)).toEqual(slugs);
    expect(cache.models.every((row: { multi_agent_version: string }) => row.multi_agent_version === "v1")).toBe(true);
  });
