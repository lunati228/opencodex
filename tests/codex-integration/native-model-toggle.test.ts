...
describe("native GPT model toggles (bare slugs in disabledModels)", () => {
  ...
  test("Direct keeps static GPT-5.6 rows visible when roster discovery is unconfirmed", () => {
    ...
    expect(poolRows).toContain("gpt-5.6-sol");
    expect(poolRows).toContain("gpt-5.6-terra");
    expect(poolRows).toContain("gpt-5.6-luna");
  });
  ...
  test("a per-model soft budget lowers compaction without changing native hard limits", () => {
    ...
    expect(nativeModelRows(oversized).find(item => item.slug === "gpt-5.6-sol"))
      .toMatchObject({ contextWindow: 272_000, maxInputTokens: 272_000, autoCompactTokenLimit: 232_560 });
  });
  ...
  test("native metadata helpers trust only marked, well-shaped account rows", () => {
    ...
    expect(trusted).toMatchObject({
      context_window: 272_000,
      max_context_window: 272_000,
      auto_compact_token_limit: autoCompactLimitFor(272_000),
    });
