...
describe("routed reasoning picker fidelity", () => {
  test("adds validation-safe product tiers to routed reasoning ladders", () => {
  ...
  test("preserves degraded routed ladders with their synthetic product tiers", () => {
    ...
    expect(merged.find(entry => entry.slug === "google-antigravity/gemini-3.7-flash")
      ?.default_reasoning_level).toBe("max");
    expect(efforts("example/gpt-5.6-sol"))
      .toEqual(["low", "medium", "high", "xhigh", "max", "ultra"]);
  });
});
...
describe("Codex catalog routed normalization", () => {
  ...
  test("an unknown window ignores the configured soft budget instead of treating 128k as policy evidence", async () => {
    ...
    expect(emitted).toMatchObject({
      context_window: 128_000,
      max_context_window: 128_000,
      auto_compact_token_limit: 109_440,
    });
  });
