...
export function startServer(port?: number, deps: StartServerDeps = {}): Server<WsData> {
    ...
    const serveOptions = {
      ...
      if (url.pathname === "/v1/models" && req.method === "GET") {
        ...
        if (url.searchParams.has("client_version")) {
          ...
          applyExternalAutoReviewCatalogPolicy(
            entries,
            new Set(goOrdered.map(catalogModelSlug)),
            config,
          );
          return jsonResponse({
  ...
  if (
    config.localRuntime?.enabled === true
    && config.localRuntime.autoStart === true
    && managedLocalRuntimeProviderIsValid(config)
  ) {
    getManagedLocalRuntimeSupervisor().requestStart(config);
  }
  if (config.localRuntime?.enabled === true) {
    startManagedLocalRuntimeIdleSweep(config);
  }
  ...
  import("../codex/catalog/quota-decoration")
    .then(({ primeCatalogQuotaDecoration }) => primeCatalogQuotaDecoration(config))
    .catch(() => {});
