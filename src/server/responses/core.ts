...
import { isCanonicalOpenAiForwardProvider, OPENAI_CODEX_PROVIDER_ID, supportsNativeResponsesCompactEndpoint } from "../../providers/openai-tiers";
...
} from "../../codex/helper-turn-models";
import { getCachedProviderQuotaReports } from "../../providers/quota";
import {
...
async function applyFinalRouteRequestNormalization(args: {
...

  {
    ...
    // parseRequest applies the upstream Codex boundary (ultra -> max) to the
    // normalized options, but native passthrough serializes _rawBody verbatim.
    // Reconcile that second representation even when max is already a valid rung
    // and nativeEffortClamp therefore has no further downgrade to report.
    if (raw?.reasoning?.effort === "ultra" && parsed.options.reasoning === "max") {
      raw.reasoning.effort = "max";
    }
...
async function handleResponsesInner(
  ...
  let localRuntimeUseLease: ManagedLocalRuntimeUseLease | undefined;
  try {
  ...
  const retainManagedLocalRuntimeUse = (candidate: RouteResult): void => {
    ...
    localRuntimeUseLease?.release();
    localRuntimeUseLease = undefined;
  };
  const trackLocalRuntimeStreamLifetime = (
    body: ReadableStream<Uint8Array>,
    ac: AbortController,
    onDone?: () => void,
    lease?: AdmissionLease,
  ): ReadableStream<Uint8Array> => {
    const localLease = localRuntimeUseLease;
    const tracked = trackStreamLifetime(
      body,
      ac,
      localLease ? localRuntimeUseOnDone(localLease, onDone) : onDone,
      lease,
    );
    if (localLease) localRuntimeUseLease = undefined;
    return tracked;
  };

  // Freeze one cache snapshot/clock reading so source classification, route
  // selection, logging, and reasoning effort cannot disagree at the exact
  // 30-minute freshness boundary.
  const helperQuotaContext = {
    reports: getCachedProviderQuotaReports(),
    now: Date.now(),
  };

  // Keep track of the selector whose route required the managed local runtime. If selection
  ...
  const selectInitialRoute = () => {
    const originalModelId = parsed.modelId;
    let effectiveModelId = originalModelId;
    ...
    // Resolve both sides before intercepting. A source and target that land on the same physical
    // provider/model pair are a no-op, while equal slugs on different providers still reroute.
    const shadowConfig = config.shadowCallIntercept;
    if (
      shadowConfig?.enabled
      && shadowConfig.model
      && isShadowSourceModel(originalModelId, shadowConfig.sourceModels)
    ) {
      const sourcePrefix = shadowSourceModelPrefix(originalModelId, shadowConfig.sourceModels)!;
      let sourceIdentity = { providerName: OPENAI_CODEX_PROVIDER_ID, modelId: sourcePrefix };
      try {
        const resolvedSource = routeConcreteModel(config, originalModelId);
        sourceIdentity = { providerName: resolvedSource.providerName, modelId: sourcePrefix };
      } catch {
        // Native Codex helper calls remain OpenAI-owned without an enabled OpenAI route.
      }
      const targetRoute = resolveRoute(shadowConfig.model);
      if (shouldInterceptShadowCall(
        originalModelId,
        shadowConfig.sourceModels,
        sourceIdentity,
        targetRoute,
      )) {
        effectiveModelId = shadowConfig.model;
        shadowRoute = targetRoute;
        shadowRewrite = {
          originalModelId,
          targetModelId: shadowConfig.model,
          sourcePrefix,
        };
      }
    }

    // Compaction has no helper slug. Classify the conversation's effective source route without
    // requiring a stopped local engine, then redirect only when the configured policy owns it.
    let compactionSourceIsExternal = false;
    if (parsed._compactionRequest === true) {
      try {
        const sourceRoute = routeModelForPolicy(
          config,
          effectiveModelId,
          undefined,
          helperQuotaContext,
        );
        compactionSourceIsExternal = !supportsNativeResponsesCompactEndpoint(
          sourceRoute.providerName,
          sourceRoute.provider,
        );
      } catch {
        // The ordinary route below owns the user-visible routing error.
      }
    }

    const activeCompactionOverride = activeAutoCompactModelOverrideForTurn(
      config,
      parsed._compactionRequest === true,
      compactionSourceIsExternal,
      helperQuotaContext,
    );
    const routingModelId = activeCompactionOverride ?? effectiveModelId;
    const redirectedCompaction = activeCompactionOverride !== undefined;
    const redirectedReview = activeAutoReviewModelOverrideForTurn(
      config,
      effectiveModelId,
      helperQuotaContext,
    ) !== undefined;
    ...
    return {
      route,
      shadowRewrite,
      helperSourceModel: effectiveModelId,
      helperTurn: redirectedCompaction
  ...
  let selection: ReturnType<typeof selectInitialRoute>;
  try {
    ...
    if (err instanceof LocalRuntimeUnavailableError) {
      localRuntimeUseLease ??= acquireManagedLocalRuntimeUse();
      const requestedLocalContext = qwenContextVariantForModelId(
        attemptedLocalModelId,
      ...
      try {
        ...
        return formatErrorResponse(
          retryError instanceof LocalRuntimeUnavailableError ? 503 : 404,
          retryError instanceof LocalRuntimeUnavailableError ? "server_error" : "invalid_request_error",
  ...
  if (selection.shadowRewrite) {
    parsed.modelId = selection.shadowRewrite.targetModelId;
    if (parsed._rawBody && typeof parsed._rawBody === "object") {
      (parsed._rawBody as { model?: string }).model = selection.shadowRewrite.targetModelId;
    }
    // Persist the operator-configured prefix, never the caller-controlled model suffix.
    logCtx.shadowCallRewrittenFrom = sanitizeLogMetadataString(selection.shadowRewrite.sourcePrefix);
  ...
  if (selection.helperTurn) {
    logCtx.helperTurn = selection.helperTurn;
    logCtx.helperSourceModel = selection.helperSourceModel;
    const effort = helperTurnReasoningEffort(config);
    if (effort) {
      parsed.options.reasoning = effort;
      const raw = parsed._rawBody as Record<string, unknown> | undefined;
      if (raw) {
        const current = raw.reasoning;
        raw.reasoning = current && typeof current === "object" && !Array.isArray(current)
          ? { ...(current as Record<string, unknown>), effort }
          : { effort };
      }
      logCtx.helperReasoningEffort = effort;
    }
  }
  logCtx.routeDecision = route.routeDecision;

  retainManagedLocalRuntimeUse(route);
  ...
  // A subagent fallback or recovered task can replace the first route. Keep a lease only for the
  // final managed-local route, then apply its selected Qwen context before it enters pacing.
  retainManagedLocalRuntimeUse(route);
  const finalLocalContext = qwenContextVariantForModelId(
    route.modelId,
    QWEN_PROFILE.modelId,
  )?.contextWindow;
  ...
  if ("passthrough" in adapter && adapter.passthrough && !routedCompaction) {
    ...
    const connectMs = effectiveLocalRuntimeConnectTimeoutMs(
   ...
   if (usesCodexForwardPoolAuth(authCtx, route.provider)) {
    ...
    if (upstreamResponse.status >= 300 && upstreamResponse.status < 400) {
      ...
      const responseBody = localRuntimeUseLease && upstreamBody
        ? trackLocalRuntimeStreamLifetime(
            upstreamBody,
            new AbortController(),
            undefined,
            options.turnAdmissionLease,
    ...
    if (headers.get("content-type")?.toLowerCase().includes("application/json")) {
      ...
      if (clientRequestedStream === true
        ...
        if (completed) {
          ...
          const responseStream = localRuntimeUseLease
            ? trackLocalRuntimeStreamLifetime(
                stream,
                new AbortController(),
                undefined,
    ...
    const turnAc = new AbortController();
    const tracked = body
      ? trackLocalRuntimeStreamLifetime(body, turnAc, undefined, options.turnAdmissionLease)
      : null;
  ...
  if ((imgPlan || vidPlan) && (!wsPlan || adapter.runTurn)) {
    ...
    const imgResponse = await runWithImageBridge({
      ...
      connectTimeoutMs: effectiveLocalRuntimeConnectTimeoutMs(
        config.connectTimeoutMs,
        route.providerName,
        route.provider,
      ),
    ...
    if (imgResponse.body) {
      ...
      return new Response(trackLocalRuntimeStreamLifetime(imgResponse.body, imgTurnAc, undefined, options.turnAdmissionLease), {
  ...
  if (canRunWebSearch && wsPlan) {
    ...
    if (!wsResponse.ok && !options.comboAttempt && !isFixedCodexAccount(authCtx)) {
      recordSubagentQuotaFailureForThreadSpawn(
        req.headers,
        subagentQuotaFailureModel,
        wsResponse.status,
    ...
    if (wsResponse.body) {
      ...
      return new Response(trackLocalRuntimeStreamLifetime(wsResponse.body, wsTurnAc, undefined, options.turnAdmissionLease), {
  ...
  if (adapter.runTurn) {
    ...
    if (parsed.stream) {
      ...
      const trackedSse = trackLocalRuntimeStreamLifetime(sseStream, bridgeTurnAc, undefined, options.turnAdmissionLease);
  ...
  if (parsed.stream) {
    ...
    const bridgeTurnAc = new AbortController();
    const trackedSse = trackLocalRuntimeStreamLifetime(
      sseStream,
      bridgeTurnAc,
      cleanupUpstreamAbort,
      options.turnAdmissionLease,
