...
async function handleResponsesInner(
  ...
  // Compaction has no helper slug. Classify the conversation's original route
  // without requiring a stopped local engine to be running, then redirect only
  // when the configured policy owns that source.
  ...
  const helperSourceModel = parsed.modelId;
  ...
  const routingModelId = activeCompactionOverride ?? parsed.modelId;
  ...
  if (redirectedCompaction || redirectedReview) {
    logCtx.helperTurn = redirectedCompaction ? "auto-compact" : "auto-review";
    logCtx.helperSourceModel = helperSourceModel;
  ...
  try {
    route = options.comboAttempt
      ? routeConcreteModel(config, routingModelId)
      : routeModel(
        config,
        routingModelId,
