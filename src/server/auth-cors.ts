...
const PROVIDER_CONFIG_FIELD_POLICY = {
  ...
  fastWire: "editor",
  baseUrl: "editor",
  externalProviderRef: "runtime",
  externalProviderState: "runtime",
  externalProviderReason: "runtime",
  localRuntimeProfileId: "runtime",
...
export function safeConfigDTO(config: OcxConfig): unknown {
  ...
  for (const [name, provider] of Object.entries(config.providers)) {
    ...
    if (isExternalProviderProjection(name, provider)) {
      dto.externalProviderRef = provider.externalProviderRef;
      dto.externalProviderState = provider.externalProviderState;
      if (provider.externalProviderReason !== undefined) {
        dto.externalProviderReason = provider.externalProviderReason;
      }
    }
  ...
  const externalProviderBundles = (config.externalProviderBundles ?? [])
    .filter(ref => EXTERNAL_PROVIDER_BUNDLE_REFS.includes(
      ref as typeof EXTERNAL_PROVIDER_BUNDLE_REFS[number],
    ));
  return {
    port: config.port,
    ...
    ...(externalProviderBundles.length > 0 ? { externalProviderBundles } : {}),
    ...(config.localRuntime ? {
      localRuntime: {
        enabled: config.localRuntime.enabled,
        autoStart: config.localRuntime.autoStart,
        profileId: config.localRuntime.profileId,
        nCtx: config.localRuntime.nCtx,
        ...(config.localRuntime.verifiedAt
          ? { verifiedAt: config.localRuntime.verifiedAt }
          : {}),
      },
    } : {}),
