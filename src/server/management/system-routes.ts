...
export async function handleSystemRoutes(ctx: ManagementContext): Promise<Response | null> {
  ...
  if (url.pathname === "/api/system/restart" && req.method === "POST") {
    ...
    const requireIdle = req.headers.get(SYSTEM_RESTART_REQUIRE_IDLE_HEADER) === "1";
    // The coordinator checks active turns and begins draining synchronously,
    // making the idle decision atomic with admission closure.
    const result = acceptSystemRestart(undefined, requireIdle);
    if (!result.accepted && result.busy) {
      return jsonResponse({
        success: false,
        error: "Proxy is busy; companion recycle deferred.",
