...
import { isExternalProviderProjection } from "../../providers/external-bundles";
import { isManagedLocalProviderProjection } from "../../local-runtime/profile";
...
export async function handleProviderRoutes(ctx: ManagementContext): Promise<Response | null> {
  ...
  if (url.pathname === "/api/provider-quotas" && req.method === "GET") {
    ...
    const reports = await fetchProviderQuotaReports(config, forceRefresh);
    // Metadata-only rewrite: quota refresh must not trigger provider discovery or
    // alter catalog membership, but the next picker open should see fresh suffixes.
    refreshCatalogQuotaDecoration(reports);
  ...
  if (url.pathname === "/api/providers" && req.method === "GET") {
    return jsonResponse(Object.entries(config.providers).map(([name, p]) => {
      const external = isExternalProviderProjection(name, p);
      const localRuntime = isManagedLocalProviderProjection(name, p);
      return {
        ...
        ...(name === "openai" && isCanonicalOpenAiForwardProvider(p)
          ...
          : {}),
        ...(external ? {
          externalProviderRef: p.externalProviderRef,
          externalProviderState: p.externalProviderState,
          ...(p.externalProviderReason ? { externalProviderReason: p.externalProviderReason } : {}),
        } : {}),
        ...(localRuntime ? { localRuntimeProfileId: p.localRuntimeProfileId } : {}),
      };
    }));
  }
  ...
  if (url.pathname === "/api/providers" && req.method === "POST") {
    ...
    if (
      ...
      && existing
      && (
        isExternalProviderProjection(name, existing)
        || isManagedLocalProviderProjection(name, existing)
      )
    ) {
      return jsonResponse({ error: "managed providers cannot be overwritten" }, 409);
    }
  ...
  if (url.pathname === "/api/providers" && req.method === "PATCH") {
    ...
    if (
      isExternalProviderProjection(name, config.providers[name]!)
      || isManagedLocalProviderProjection(name, config.providers[name]!)
    ) {
      return jsonResponse({ error: "managed providers are read-only" }, 409);
    }
    let rawBody: unknown;
  ...
  if (url.pathname === "/api/providers/test" && req.method === "POST") {
    ...
    if (
      prov.externalProviderRef !== undefined
      && (
        !isExternalProviderProjection(name, prov)
        || prov.externalProviderState !== "ready"
      )
    ) {
      return jsonResponse({ ok: false, error: "Managed external provider is unavailable", latencyMs: 0 });
    }
    if (prov.disabled) {
  ...
  if (url.pathname === "/api/providers" && req.method === "DELETE") {
    ...
    if (
      ...
      return jsonResponse({ error: "managed providers cannot be deleted here" }, 409);
    }
