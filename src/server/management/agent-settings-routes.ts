...
export async function handleAgentSettingsRoutes(ctx: ManagementContext): Promise<Response | null> {
  ...
  if (url.pathname === "/api/v2" && req.method === "PUT") {
    ...
    if (wantsFlag && body.enabled === true && (hybridPinActive || effectiveMode === "v1")) {
      return jsonResponse({
        error: "body.enabled=true conflicts with the saved V1 catalog policy: Codex's global multi_agent_v2 override outranks catalog pins",
      }, 400);
    }
