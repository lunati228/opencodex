...
import { EXTERNAL_PROVIDER_BUNDLE_REFS } from "../../providers/external-bundles";
import { providerCodexAccountMode } from "../../providers/registry";
import { routedSlug, slugEquals } from "../../providers/slug-codec";
import { clearProviderQuotaCache, fetchProviderQuotaReports } from "../../providers/quota";
import { isCanonicalOpenAiForwardProvider } from "../../providers/openai-tiers";
import { routeModelForPolicy } from "../../router";
...
import { helperTurnCodexRemainingPercentThreshold } from "../../codex/helper-turn-models";
...
export async function handleConfigRoutes(ctx: ManagementContext): Promise<Response | null> {
  ...
  // The bundled NVIDIA providers (GLM, DeepSeek, Kimi) ship as inert `.disabled.json`
  // credential files and only become providers once their ref is listed in
  // `externalProviderBundles`. Nothing could write that list: full config PUT is refused
  // above and no other route touches it, so the bundles were unreachable however correct the
  // credential file was — the same shape of gap as localRuntime.autoStart.
  //
  // This does not decrypt, move or rename a credential file. It records which bundles the
  // operator opted into; `applyExternalProviderBundles` still validates each one on load and
  // is the only thing that decides whether a bundle ends up routable.
  if (url.pathname === "/api/external-bundles" && req.method === "GET") {
    return jsonResponse({
      enabled: config.externalProviderBundles ?? [],
      available: EXTERNAL_PROVIDER_BUNDLE_REFS,
    });
  }

  if (url.pathname === "/api/external-bundles" && req.method === "PUT") {
    let raw: unknown;
    try {
      raw = await req.json();
    } catch {
    ...
    if (!isPlainRecord(raw) || !Array.isArray(raw.refs) || Object.keys(raw).some(key => key !== "refs")) {
      return jsonResponse({ error: "body must be exactly { refs: string[] }" }, 400);
    }
    const known = new Set<string>(EXTERNAL_PROVIDER_BUNDLE_REFS);
    const refs: string[] = [];
    for (const ref of raw.refs) {
      // Reject rather than skip: silently dropping an unknown ref would report success for a
      // provider that is never going to appear.
      if (typeof ref !== "string" || !known.has(ref)) {
        return jsonResponse({ error: `unknown external bundle ref: ${String(ref)}` }, 422);
      }
      if (!refs.includes(ref)) refs.push(ref);
    }
    config.externalProviderBundles = refs;
    (deps.saveConfigPreservingClaudeCode ?? saveConfigPreservingClaudeCode)(config);
    // The picker list is derived from providers, which this changes.
    invalidateCodexModelsCache();
    return jsonResponse({ enabled: refs });
  }

  // Codex's two automatic helper turns. See src/codex/helper-turn-models.ts for why each one
  // needed a different signal to redirect.
  if (url.pathname === "/api/helper-turn-models" && req.method === "GET") {
    return jsonResponse({
      autoReviewModel: config.autoReviewModel ?? null,
      autoCompactModel: config.autoCompactModel ?? null,
      helperTurnScope: config.helperTurnScope ?? "all",
      helperTurnReasoningEffort: config.helperTurnReasoningEffort ?? null,
      helperTurnCodexRemainingPercentThreshold:
        helperTurnCodexRemainingPercentThreshold(config) ?? null,
    });
  }

  if (url.pathname === "/api/helper-turn-models" && req.method === "PUT") {
    let raw: unknown;
    try {
      raw = await req.json();
    } catch {
    ...
    const fields = ["autoReviewModel", "autoCompactModel"] as const;
    ...
    if (!isPlainRecord(raw) || Object.keys(raw).some(key => !allowedFields.has(key))) {
      return jsonResponse({
        error: "body may contain only autoReviewModel, autoCompactModel, "
          + "helperTurnScope, helperTurnReasoningEffort, and "
          + "helperTurnCodexRemainingPercentThreshold",
      }, 400);
    }
    const hasScope = Object.hasOwn(raw, "helperTurnScope");
    let nextScope: "external" | undefined;
    if (hasScope) {
      if (raw.helperTurnScope === null || raw.helperTurnScope === "all") {
        nextScope = undefined;
      } else if (raw.helperTurnScope === "external") {
        nextScope = "external";
      } else {
        return jsonResponse({ error: "helperTurnScope must be all, external, or null" }, 400);
      }
    }
    const hasEffort = Object.hasOwn(raw, "helperTurnReasoningEffort");
    let nextEffort: string | undefined;
    if (hasEffort) {
      const effort = raw.helperTurnReasoningEffort;
      if (effort === null) {
        nextEffort = undefined;
      } else if (typeof effort === "string" && isCodexReasoningEffort(effort)) {
        nextEffort = effort;
      } else {
        return jsonResponse({
          error: "helperTurnReasoningEffort must be a Codex effort label or null",
        }, 400);
      }
    }
    const hasQuotaThreshold = Object.hasOwn(raw, "helperTurnCodexRemainingPercentThreshold");
    let nextQuotaThreshold: number | undefined;
    if (hasQuotaThreshold) {
      const threshold = raw.helperTurnCodexRemainingPercentThreshold;
      if (threshold === null) {
        nextQuotaThreshold = undefined;
      } else if (
        ...
        nextQuotaThreshold = threshold;
      } else {
        return jsonResponse({
          error: "helperTurnCodexRemainingPercentThreshold must be an integer from 0 to 100 or null",
        }, 400);
      }
    }
    const resolved: Partial<Record<typeof fields[number], string | undefined>> = {};
    for (const field of fields) {
      if (!(field in raw)) continue;
      const value = raw[field];
      // null clears the override and restores the default behaviour for that turn.
      if (value === null) {
        resolved[field] = undefined;
        continue;
      }
      if (typeof value !== "string" || value.trim().length === 0) {
        return jsonResponse({ error: `${field} must be a non-empty string or null` }, 400);
      }
      const target = value.trim();
      // Validate here rather than at request time: a helper turn fires without the user
      // watching, so a mistake would surface as a mid-conversation failure.
      //
      // routeModel() succeeding is NOT proof the model exists — an unrecognised id falls
      // through every rule to `defaultProvider`, so "nope/not-a-model" would "route" fine. The
      // one thing a namespaced id does prove is its namespace, so require that namespace to
      // resolve to exactly one configured provider key or case-insensitive alias. A bare slug
      // keeps the router's own permissive behaviour; this route does not invent a stricter
      // contract for it than routing itself has.
      const slash = target.indexOf("/");
      if (slash > 0) {
        const namespace = target.slice(0, slash);
        const aliasMatches = hasOwnProvider(config.providers, namespace)
          ? 1
          : Object.values(config.providers).filter(provider =>
            typeof provider.alias === "string"
            && provider.alias.toLowerCase() === namespace.toLowerCase()
          ).length;
        if (aliasMatches !== 1) {
          return jsonResponse({ error: `${field} names no configured provider: ${namespace}` }, 422);
        }
      }
      let route: ReturnType<typeof routeModelForPolicy>;
      try {
        route = routeModelForPolicy(config, target);
      } catch (err) {
        return jsonResponse({ error: `${field} does not route: ${err instanceof Error ? err.message : String(err)}` }, 422);
      }
      // Compaction on the canonical ChatGPT forward path returns an OpenAI-ENCRYPTED summary
      // blob. Codex stores it and replays it to the conversation model, which for anything
      // routed decodes to nothing (compactionItemToText -> OPAQUE_COMPACTION_NOTE). Allowing it
      // would silently throw away the history it was supposed to preserve, so it fails closed.
      if (field === "autoCompactModel" && isCanonicalOpenAiForwardProvider(route.provider)) {
        return jsonResponse({
          error: "autoCompactModel cannot be a ChatGPT-forward model: its summary is encrypted "
            + "and unreadable by every other model. Pick a routed model.",
        }, 422);
      }
      resolved[field] = target;
    }
    if (Object.hasOwn(resolved, "autoReviewModel")) {
      if (resolved.autoReviewModel === undefined) deleteConfigTopLevelKey(config, "autoReviewModel");
      else config.autoReviewModel = resolved.autoReviewModel;
    }
    if (Object.hasOwn(resolved, "autoCompactModel")) {
      if (resolved.autoCompactModel === undefined) deleteConfigTopLevelKey(config, "autoCompactModel");
      else config.autoCompactModel = resolved.autoCompactModel;
    }
    if (hasScope) {
      if (nextScope === undefined) deleteConfigTopLevelKey(config, "helperTurnScope");
      else config.helperTurnScope = nextScope;
    }
    if (hasEffort) {
      if (nextEffort === undefined) deleteConfigTopLevelKey(config, "helperTurnReasoningEffort");
      else config.helperTurnReasoningEffort = nextEffort;
    }
    if (hasQuotaThreshold) {
      if (nextQuotaThreshold === undefined) {
        deleteConfigTopLevelKey(config, "helperTurnCodexRemainingPercentThreshold");
      }
      else config.helperTurnCodexRemainingPercentThreshold = nextQuotaThreshold;
    }
    (deps.saveConfigPreservingClaudeCode ?? saveConfigPreservingClaudeCode)(config);
    return jsonResponse({
      autoReviewModel: config.autoReviewModel ?? null,
