...
export async function handleConfigRoutes(ctx: ManagementContext): Promise<Response | null> {
  ...
  if (url.pathname === "/api/helper-turn-models" && req.method === "PUT") {
    ...
    for (const field of fields) {
      ...
      // one thing a namespaced id does prove is its prefix, so check that explicitly. A bare
      // slug keeps the router's own permissive behaviour; this route does not invent a stricter
      // contract for it than routing itself has.
      const slash = target.indexOf("/");
      if (slash > 0 && !Object.prototype.hasOwnProperty.call(config.providers, target.slice(0, slash))) {
        return jsonResponse({ error: `${field} names no configured provider: ${target.slice(0, slash)}` }, 422);
      }
    ...
    for (const [field, target] of Object.entries(resolved)) {
      if (target === undefined) delete config[field as typeof fields[number]];
      else config[field as typeof fields[number]] = target;
    }
    if (hasScope) {
      if (nextScope === undefined) delete config.helperTurnScope;
      else config.helperTurnScope = nextScope;
    }
    if (hasEffort) {
      if (nextEffort === undefined) delete config.helperTurnReasoningEffort;
      else config.helperTurnReasoningEffort = nextEffort;
    }
    if (hasQuotaThreshold) {
      if (nextQuotaThreshold === undefined) delete config.helperTurnCodexRemainingPercentThreshold;
