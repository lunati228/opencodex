...
export function requestLogEntryFromPersistedUsage(entry: PersistedUsageEntry): RequestLogEntry {
  ...
  const helperSourceModel = sanitizeLogMetadataString(entry.helperSourceModel);
  const helperReasoningEffort = sanitizeLogMetadataString(entry.helperReasoningEffort);
...
export function addRequestLog(entry: RequestLogEntry) {
  ...
  const helperTurn = entry.helperTurn === "auto-review" || entry.helperTurn === "auto-compact"
    ? entry.helperTurn
    : undefined;
  const helperSourceModel = sanitizeLogMetadataString(entry.helperSourceModel);
  const helperReasoningEffort = sanitizeLogMetadataString(entry.helperReasoningEffort);
  const shadowCallRewrittenFrom = sanitizeLogMetadataString(entry.shadowCallRewrittenFrom);
  const claudeCompatibility = normalizeClaudeCompatibilityUsageLog(entry.claudeCompatibility);
  const metadataChanged = entry.claudeCompatibility !== undefined || helperTurn !== entry.helperTurn
    || helperSourceModel !== entry.helperSourceModel
    || helperReasoningEffort !== entry.helperReasoningEffort
    || shadowCallRewrittenFrom !== entry.shadowCallRewrittenFrom;
  const retained: RequestLogEntry = metadataChanged ? { ...entry } : entry;
  if (metadataChanged) {
    if (helperTurn) retained.helperTurn = helperTurn;
    else delete retained.helperTurn;
    if (helperSourceModel) retained.helperSourceModel = helperSourceModel;
    else delete retained.helperSourceModel;
    if (helperReasoningEffort) retained.helperReasoningEffort = helperReasoningEffort;
    else delete retained.helperReasoningEffort;
...
export function addFinalRequestLog(
  ...
  const helperSourceModel = sanitizeLogMetadataString(logCtx.helperSourceModel);
  const helperReasoningEffort = sanitizeLogMetadataString(logCtx.helperReasoningEffort);
