...
export function noteSubagentModelFailure(
  ...
  const quotaFailure = isRateLimitOrQuotaFailureMessage(message);
  if (!quotaFailure && !isPermanentModelAvailabilityFailureMessage(message)) return;
  const route = tryRouteFallbackModel(config, model);
  const poolScoped = !!route && isPoolCodexRoute(route);
  modelHealth.set(
    healthKey(
    ...
    ),
    {
      ...
      reason: quotaFailure ? "quota_exhausted" : "model_unavailable",
    },
  );
  sweepExpiredOnWrite(now);
}

function isPermanentModelAvailabilityFailureMessage(message: string): boolean {
  const normalized = String(message ?? "").trim();
  if (!normalized) return false;
  const numericStatus = Number(normalized);
  if (numericStatus === 404 || numericStatus === 410) return true;
  const lower = normalized.toLowerCase();
  if (/\bprovider error\s+(?:404|410)\b/.test(lower)) return true;
  if (/\b(?:model not found|model unavailable|unsupported model)\b/.test(lower)) return true;
  return /\bmodel\b.{0,200}\b(?:(?:has )?reached (?:its )?end of life|is no longer available|has been retired|was retired)\b/s.test(lower);
}
