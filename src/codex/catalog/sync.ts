...
import { qwenContextVariantDisplayName, qwenContextVariantForModelId } from "../../local-runtime/context-tiers";
import { isAgyBridgeRowSlug } from "./agy-bridge-row";
import { normalizePersistedPickerRows } from "./persisted-picker-rows";
import { externalAutoReviewCatalogOverride } from "../helper-turn-models";
...
import { createReserveCatalogProjection, isReserveCatalogProjection, RESERVE_LUNA_METADATA_SOURCE, RESERVE_SOURCE_CATALOG_FIELD, type ReserveCatalogProjection } from "./reserve";
...
/** Managed local rows keep the runtime/template's exact reasoning vocabulary. */
function managedLocalProfileForCatalogModel(
  providerId: string,
  modelId: string,
): typeof QWEN_PROFILE | undefined {
  return providerId === QWEN_PROFILE.providerId
    && qwenContextVariantForModelId(modelId, QWEN_PROFILE.modelId)
    ? QWEN_PROFILE
    : undefined;
}

function managedLocalProfileForCatalogSlug(
  slug: unknown,
): typeof QWEN_PROFILE | undefined {
  if (typeof slug !== "string") return undefined;
  const slash = slug.indexOf("/");
  if (slash <= 0) return undefined;
  return managedLocalProfileForCatalogModel(
    slug.slice(0, slash),
    slug.slice(slash + 1),
  );
}

function managedLocalPickerEfforts(
  profile: typeof QWEN_PROFILE,
): string[] {
...
/** Apply display-only quota and managed Qwen context metadata to a routed row. */
export function applyPickerAffordances(entry: RawEntry, providerId: string): void {
  if (!providerId) return;
  if (typeof entry.description === "string") {
    entry.description = withQuotaSuffix(
      entry.description,
      quotaSuffixForProvider(getCachedProviderQuotaReports(), providerId),
    );
  }
  if (providerId !== QWEN_PROFILE.providerId || typeof entry.slug !== "string") return;
  const variant = qwenContextVariantForModelId(entry.slug, QWEN_PROFILE.modelId);
  if (!variant) return;
  entry.display_name = qwenContextVariantDisplayName(variant);
  entry.context_window = variant.contextWindow;
  entry.max_context_window = variant.contextWindow;
  entry.auto_compact_token_limit = variant.autoCompactTokenLimit;
...
/** Mark only routed conversation rows with the fork-owned external Guardian alias. */
export function applyExternalAutoReviewCatalogPolicy(
  entries: RawEntry[],
  routedSlugs: ReadonlySet<string>,
  config: OcxConfig,
): void {
  const override = externalAutoReviewCatalogOverride(config);
  if (!override) return;
  for (const entry of entries) {
    if (typeof entry.slug === "string" && routedSlugs.has(entry.slug)) {
      entry.auto_review_model_override = override;
    }
  }
}
...
export function deriveEntry(
  ...
  const managedLocalProfile = model
    ? managedLocalProfileForCatalogModel(model.provider, model.id)
    : undefined;
  ...
  if (template || codexForwardNativeCapabilityAlias) {
    ...
    if (isRouted) {
      ...
      applyReasoningLevels(
        ...
        preserveExactReasoning || codexForwardNativeCapabilityAlias !== null || managedLocalProfile !== undefined,
  ...
  if (isRouted) {
    ...
    applyReasoningLevels(
      ...
      model?.reasoningEfforts,
      model?.defaultReasoningEffort,
      preserveExactReasoning || managedLocalProfile !== undefined,
      managedLocalProfile !== undefined,
    );
  }
  else {
...
export function buildCatalogEntriesFromObservedState({
  ...
  for (const m of goModels) {
    ...
    applyPickerAffordances(e, m.provider);
    out.push(e);
  }
...
export function mergeCatalogEntriesFromObservedState({
  ...
  const mergedEntries = [...native, ...managedEntries].map(m => {
    ...
    const managedLocalProfile = managedLocalProfileForCatalogSlug(e.slug);
    if (managedLocalProfile) {
      // A degraded provider observation can retain this row from disk. Repair
      // its label and native ladder instead of reviving stale synthetic tiers.
      applyReasoningLevels(
        e,
        managedLocalPickerEfforts(managedLocalProfile),
        managedLocalProfile.defaultReasoningEffort,
        true,
        true,
      );
    ...
    } else if (!freshCustomEntries.has(m) && !exactCombo && !reserveProjection && !String(e.slug ?? "").startsWith("opencode-go/")) {
  ...
  return normalizePersistedPickerRows(versionedEntries);
}
...
function writeRetainedCatalogSync({
  ...
  applyExternalAutoReviewCatalogPolicy(
    goEntries,
    new Set(orderedGoModels.map(catalogModelSlug)),
    config,
  );
...
export function invalidateCodexModelsCacheWithPermit(
  ...
  try {
    ...
    const cachedModels = [...models, ...observedAccountModels];
    // Bootstrap can refresh a cache before the next full catalog gather. A saved
    // V1 policy must also cover this path, even if the retained catalog predates it.
    if (currentConfig.multiAgentMode === "v1") applyMultiAgentMode(cachedModels, "v1");
    const wrapper = {
