...
import { externalProviderModelDisplayName } from "../../providers/external-bundles";
import {
...
async function fetchProviderModelsWithAuth(
  ...
  const configured: CatalogModel[] = configuredIds.map(id => ({
    id,
    provider: name,
    ...(externalProviderModelDisplayName(name, id)
      ? { displayName: externalProviderModelDisplayName(name, id) }
      : {}),
  ...
  const customModels = (config.customModels ?? []).map(cm => {
    ...
    const base: CatalogModel = {
      ...
      ...(cm.displayName
        ...
        : externalProviderModelDisplayName(cm.provider, cm.modelId)
          ? { displayName: externalProviderModelDisplayName(cm.provider, cm.modelId) }
