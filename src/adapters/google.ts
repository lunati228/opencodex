...
import {
  antigravityInferenceUrl,
  isCanonicalAntigravityBaseUrl,
} from "../providers/antigravity-endpoint";
...
export function createGoogleAdapter(provider: OcxProviderConfig): ProviderAdapter {
  ...
  return {
    ...
    async buildRequest(parsed: OcxParsedRequest) {
      ...
      // Codex Guardian uses Responses `text.format` to require strict JSON. Direct Gemini's
      // current generateContent wire calls this `responseFormat`, while Vertex and the internal
      // Cloud Code Assist GenerateContent backend retain responseMimeType/responseSchema.
      // OpenAI's name/description/strict fields wrap the schema rather than belonging to it, so
      // only the actual JSON Schema crosses providers.
      ...
      if (textFormat) {
        if (provider.googleMode === "vertex" || provider.googleMode === "cloud-code-assist") {
          ...
          if (textFormat.type === "json_schema" && textFormat.schema) {
            generationConfig.responseSchema = textFormat.schema;
          }
        } else {
          generationConfig.responseFormat = {
            text: {
              mimeType: "application/json",
              ...(textFormat.type === "json_schema" && textFormat.schema
      ...
      if (!textFormat && !generationConfig.thinkingConfig && isImageCapableModel(parsed.modelId)) {
      ...
      if (provider.googleMode === "cloud-code-assist") {
        ...
        if (!isCanonicalAntigravityBaseUrl(base)) {
          throw new Error("google-antigravity requires the canonical Google endpoint");
        }
        const url = antigravityInferenceUrl(parsed.stream);
        ...
        // Current/retired Flash suffixes move their encoded tier into thinkingLevel; suffixes
        // whose wire id still owns the effort return undefined, avoiding a contradictory field.
        if (thinkingLevel) {
