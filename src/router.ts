...
import { AGY_BRIDGE_ROW_MESSAGE, isAgyBridgeRowSlug } from "./codex/catalog/agy-bridge-row";
import {
  externalProviderCollisionBlocksRouting,
...
import { managedLocalRuntimeCanRoute } from "./local-runtime/production";
...
function routeResult(
  ...
  if (externalProviderCollisionBlocksRouting(config, providerName)) {
    throw new Error(`Managed external provider bundle collision: ${providerName}`);
  }
  if (provider.disabled === true) throw new Error(`Provider is disabled: ${providerName}`);
  if (
    provider.externalProviderRef !== undefined
    && (
      !isExternalProviderProjection(providerName, provider)
      || provider.externalProviderState !== "ready"
    )
  ) {
    throw new Error(`Managed external provider is unavailable: ${providerName}`);
  }
  if (
    requireLocalReady
    && isManagedLocalProviderProjection(providerName, provider)
    && !managedLocalRuntimeCanRoute()
  ) {
...
function routeModelInternal(
  ...
  // Native review stays on Codex. Only the external Guardian alias redirects
  // when helperTurnScope is "external".
  if (
    !bypassCombos
    && (modelId === CODEX_AUTO_REVIEW_SLUG || modelId === EXTERNAL_AUTO_REVIEW_SLUG)
  ) {
    const target = autoReviewTurnModelId(config, modelId, helperQuotaContext);
    if (target !== modelId) {
  ...
  if (profile && policyId) {
    ...
    const routed = routeModelInternal(
      config,
      concrete,
      true,
      requireLocalReady,
  ...
  if (slash > 0) {
    ...
    if (binding) {
      ...
      return {
        ...routeResult(config, OPENAI_CODEX_PROVIDER_ID, provider, nativeModelId, "explicit-account", "account-namespace", requireLocalReady),
  ...
  if (slash > 0) {
    ...
    if (hasOwnProvider(config.providers, provName)) {
      ...
      if (known.includes(modelId)) {
        return routeResult(config, provName, prov, modelId, "explicit-provider", "explicit-provider-namespace", requireLocalReady);
      }
  ...
  for (const [provName, prov] of activeProviderEntries(config)) {
    if (prov.defaultModel === modelId
      ...
      return routeResult(config, provName, prov, prov.defaultModel as string, "explicit-provider", "configured-default-model", requireLocalReady);
    }
  }

  const patternRoute = routeByKnownModelPattern(config, modelId, requireLocalReady);
  ...
  for (const [provName, prov] of activeProviderEntries(config)) {
    if (prov.models && Array.isArray(prov.models)) {
      ...
      if (hit !== undefined) {
        return routeResult(config, provName, prov, hit, "explicit-provider", "configured-model-list", requireLocalReady);
      }
    }
  }
  ...
  if (aliasMatches[0]) {
    ...
    return routeResult(
      config,
      match.provider,
      config.providers[match.provider],
      match.model,
      "explicit-provider",
      "model-alias",
      requireLocalReady,
  ...
  if (hasOwnProvider(config.providers, config.defaultProvider)) {
    ...
    // Explicit routed selectors are not ChatGPT model ids. An absent provider
    // must fail locally, including stale picker entries after config cleanup.
    // Declared vendor ids and aliases have already resolved above.
    if (slash > 0 && isCanonicalOpenAiForwardProvider(defaultProv)) {
      throw new Error(`No provider configured for external model: ${modelId}`);
    }
    return routeResult(config, config.defaultProvider, defaultProv, modelId, "default-provider", "default-provider", requireLocalReady);
  }
...
/** Resolve provider/model policy without requiring a currently running local engine. */
export function routeModelForPolicy(
  config: OcxConfig,
  modelId: string,
  policyEvidence?: PolicyRequestEvidence,
  helperQuotaContext?: HelperTurnQuotaContext,
...
export function routeConcreteModel(config: OcxConfig, modelId: string): RouteResult {
  return routeModelInternal(config, modelId, true, true, undefined);
}

function routeByKnownModelPattern(
  config: OcxConfig,
  modelId: string,
  requireLocalReady: boolean,
  ...
  for (const { providerNames, prefixes } of MODEL_PROVIDER_PATTERNS) {
    if (prefixes.some(prefix => modelId.startsWith(prefix))) {
      ...
      if (matchingProvider) {
        ...
        return routeResult(config, provName, prov, modelId, "explicit-provider", "model-pattern", requireLocalReady);
      }
