...
import { getConfigDir, atomicWriteFile, atomicWriteFileAsync, backupInvalidConfig, hardenConfigDir, hardenExistingSecret } from "../config";
...
async function persist(store: AuthStore): Promise<void> {
  const dir = getConfigDir();
  ...
  hardenConfigDir();
  // OAuth mutations are serialized in-process and protected by the cross-process
  // auth-store lock below. That makes this boundary eligible for the ACL layer's
  // single bounded timeout recovery: one transient icacls stall can no longer leave
  // every later token refresh failing until the proxy restarts, while a second stall
  // remains fail-closed and cannot turn into an unbounded retry loop.
