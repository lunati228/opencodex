...
const USAGE = `Usage:
  ...
  ocx observe agy-usage [--refresh] [--json]`;
...
/** Antigravity quota. `--refresh` re-scrapes instead of serving the cached window. */
async function agyUsage(argv: string[], deps: RuntimeApiDeps): Promise<void> {
  const args = [...argv];
  const wantsJson = takeFlag(args, "--json");
  const refresh = takeFlag(args, "--refresh");
  rejectArgs(args, USAGE);
  const result = refresh
    ? await runtimeRequest("/api/agy-usage/refresh", { method: "POST" }, deps)
    : await runtimeRequest("/api/agy-usage/status", {}, deps);
  printData(result, wantsJson, summaryLines(result));
}
...
export async function handleObserveCommand(argv: string[], deps: RuntimeApiDeps = {}): Promise<number> {
  return runCliAction(async () => {
    ...
    else if (sub === "agy-usage") await agyUsage(rest, deps);
