...
import { chmodSync, constants as fsConstants, copyFileSync, existsSync, unlinkSync, readFileSync } from "node:fs";
import { dirname } from "node:path";
import { injectCodexConfig } from "../codex/inject";
import { classifyOpenAiTierBackup, getConfigPath, getDefaultConfig, hardenDirectoryForSecretWrite, isValidProviderName, readExistingSecretFileRequired, saveConfig, ConfigMutationLockError, initializePersistedConfigIfMissing, observeInitialConfigState, preserveOpenAiTierRollbackSnapshot } from "../config";
...
export function cleanupOpenAiTierBackupAfterInit(configPath = getConfigPath()): void {
  ...
  try {
    ...
    hardenDirectoryForSecretWrite(dirname(configPath));
    const backupBytes = readExistingSecretFileRequired(backup);
    if (classifyOpenAiTierBackup(backupBytes) === "stale") {
      unlinkSync(backup);
      return;
    }
    ...
    for (let attempt = 0; attempt < 16; attempt++) {
      ...
      try {
        try { chmodSync(preserved, 0o600); } catch { /* platform may ignore chmod */ }
        const preservedBytes = readExistingSecretFileRequired(preserved);
        if (!Buffer.from(preservedBytes).equals(Buffer.from(backupBytes))) {
          throw new Error("Preserved OpenAI rollback snapshot failed byte verification");
        }
      } catch (error) {
        try { unlinkSync(preserved); } catch { /* source rollback remains authoritative */ }
        throw error;
      }
      unlinkSync(backup);
