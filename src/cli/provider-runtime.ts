...
/**
 * Bundled providers ship as inert credential files and become routable only once their ref is
 * listed here. `--enable` replaces the whole list (that is the wire contract), so pass every ref
 * you want on; `--disable-all` turns them all back off.
 */
async function bundles(argv: string[], deps: RuntimeApiDeps): Promise<void> {
  const args = [...argv];
  const enable = takeOption(args, "--enable");
  const disableAll = takeFlag(args, "--disable-all");
  const wantsJson = takeFlag(args, "--json");
  if (enable !== undefined && disableAll) throw new CliUsageError("--enable and --disable-all cannot be combined", USAGE);
  rejectArgs(args, USAGE);
  if (enable === undefined && !disableAll) {
    const result = await runtimeRequest<{ enabled?: string[]; available?: string[] }>("/api/external-bundles", {}, deps);
    printData(result, wantsJson, [
  ...
  const refs = disableAll ? [] : (csv(enable) ?? []);
  const result = await runtimeRequest<{ enabled?: string[] }>("/api/external-bundles", {
    method: "PUT",
