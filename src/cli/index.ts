...
import { proxyLifecycleOwnerFromEnvironment } from "../codex/companion-ownership";
import { isCodexRoutingPreservedForExit } from "../codex/shutdown-routing";
...
async function handleStart(options: { block?: boolean } = {}) {
  ...
  const config = loadConfig();
  const lifecycleOwner = proxyLifecycleOwnerFromEnvironment(process.env);
  writeRuntimePort({
    pid: process.pid,
    port,
  ...
  const syncCleanup = () => {
    ...
    const preserveRouting = process.env.OCX_SERVICE === "1" || isCodexRoutingPreservedForExit();
