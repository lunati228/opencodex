...
const commandRunners: Record<string, CommandRunner> = {
  ...
  "codex-companion": async deps => {
    const { codexCompanionCommand } = await import("../codex/companion-runtime");
    await codexCompanionCommand(deps.args.slice(1));
    return Number(process.exitCode ?? 0);
  },
  ...
  "local-runtime": async deps => {
    const { handleLocalRuntimeCommand } = await import("./local-runtime");
    return await handleLocalRuntimeCommand(deps.args.slice(1));
  },
  route: async deps => {
