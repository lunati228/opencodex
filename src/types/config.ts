import type { LocalRuntimeProfileId, LocalRuntimeReasoningEffort, OcxProviderConfig } from "./provider";
...
export interface LocalRuntimeConfig {
  /** Enable the fixed backend-owned local runtime projection. */
  enabled: boolean;
  /** Start the last verified profile when the OpenCodex server starts. */
  autoStart: boolean;
  /** Known profile id; arbitrary executable/model profiles are intentionally unsupported. */
  profileId: LocalRuntimeProfileId;
  /** Total llama-server context allocation. */
  nCtx: number;
  /** Thinking depth. Omitted means the profile default. */
  reasoningEffort?: LocalRuntimeReasoningEffort;
  /** Set only after the exact profile reported the requested effective context. */
  verifiedAt?: string;
}
...
export interface OcxConfig {
  ...
  /** Opaque ids for fixed backend-owned external credential bundles. */
  externalProviderBundles?: string[];
  /** Durable last-known-good intent for the supervised local runtime. */
  ...
  /** Routed model used for Codex auto-review helper turns. */
  autoReviewModel?: string;
  /** Routed model used for Codex auto-compaction helper turns. */
  autoCompactModel?: string;
  /** Whether helper redirects apply globally or only to routed/non-native conversations. */
  helperTurnScope?: "all" | "external";
  /** Exact Codex effort label forced onto redirected helper turns. */
  helperTurnReasoningEffort?: string;
  /**
   * Activate the configured reviewer for native Codex turns when fresh Codex
   * quota has at most this percentage remaining. External review and compaction
   * remain separately configured; native GPT compaction is never redirected.
   */
  helperTurnCodexRemainingPercentThreshold?: number;
  /**
