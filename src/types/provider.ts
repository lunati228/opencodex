...
/** Fixed backend-owned local runtime profiles. */
export type LocalRuntimeProfileId = "qwen38-27b-q6kl";

/** Thinking depth mapped onto the managed llama.cpp runtime. */
export type LocalRuntimeReasoningEffort = "off" | "low" | "medium" | "xhigh";
...
export interface OcxProviderConfig {
  ...
  fastWire?: FastWire | null;
  baseUrl: string;
  /** Fixed opaque bundle id for a backend-only external credential source. */
  externalProviderRef?: string;
  /** Safe projected state for an externally sourced provider. */
  externalProviderState?: "ready" | "configured-unavailable" | "bundle-error";
  /** Fixed non-secret reason code for external-provider state. */
  externalProviderReason?: string;
  /** Backend-only marker for the derived, non-persistable local runtime provider. */
