...
interface AliasReference {
  ...
  alias: string;
  removableContainerPath?: readonly string[];
  /** Bun 1.3.14 preserves the TOML-trimmed newline after a multiline opener. */
  parserPreservedOpeningNewline?: boolean;
}

function parsedModelReferenceAlias(value: string): {
  alias: string;
  parserPreservedOpeningNewline: boolean;
} {
  if (value.startsWith("\r\n")) {
    return { alias: value.slice(2), parserPreservedOpeningNewline: true };
  }
  if (value.startsWith("\n")) {
    return { alias: value.slice(1), parserPreservedOpeningNewline: true };
  }
  return { alias: value, parserPreservedOpeningNewline: false };
}

function sourceHasMultilineOpeningNewline(content: string, start: number): boolean {
  const delimiter = content.slice(start, start + 3);
  if (delimiter !== '\"\"\"' && delimiter !== "'''") return false;
  return content[start + 3] === "\n"
    || (content[start + 3] === "\r" && content[start + 4] === "\n");
}
  ...
  const visit = (
    ...
    if (patternIndex === pattern.length) {
      if (typeof value === "string") {
        const parsedAlias = parsedModelReferenceAlias(value);
        references.push({
          path,
          alias: parsedAlias.alias,
          ...(parsedAlias.parserPreservedOpeningNewline
            ? { parserPreservedOpeningNewline: true }
            : {}),
...
function transformAliasReferences(
  ...
  for (const [targetIndex, target] of targets.entries()) {
    ...
    const probeCandidates = target.parserPreservedOpeningNewline
      ? pathCandidates.filter(candidate =>
        sourceHasMultilineOpeningNewline(content, candidate.valueStart))
      : pathCandidates;
    // A single-line basic string may semantically begin with an escaped newline. Bun returns
    // the same leading byte as its multiline-opening parser quirk, so source shape—not the
    // parsed value—decides whether TOML's mandatory opening-newline trim applies.
    if (target.parserPreservedOpeningNewline
      && pathCandidates.length > 0
      && probeCandidates.length === 0) continue;
