...
const EMPTY_COMPACTION_SUMMARY_ERROR = "compaction turn produced an empty summary";
const MISSING_COMPACTION_TERMINAL_ERROR = "compaction turn ended without a terminal done event";
const UNKNOWN_COMPACTION_STOP_ERROR = "compaction turn ended with an unrecognized stop reason";

const CLEAN_COMPACTION_STOP_REASONS = new Set(["stop", "end_turn", "stop_sequence", "completed"]);

function hasUsableCompactionSummary(summary: string): boolean {
  return summary.trim().length > 0;
}

function isCleanCompactionStopReason(stopReason: string | undefined): boolean {
  if (stopReason === undefined || stopReason.trim().length === 0) return true;
  return CLEAN_COMPACTION_STOP_REASONS.has(stopReason.trim().toLowerCase());
}
...
export function bridgeToResponsesSSE(
  ...
  let stepping = false;
      ...
      const emitCompactionFailure = (message: string, usage: OcxUsage | undefined): void => {
        if (compactionTextBytes > 0) {
          budget?.releaseRetained(compactionTextBytes, { kind: "retained_collectors" });
          compactionTextBytes = 0;
        }
        const failure = responseError(502, "invalid_response_error", message);
        ...
        while (!terminated && !closed && emittedFrames === emittedAtStart) {
          ...
          switch (event.type) {
            ...
            case "done": {
              ...
              // and streaming path share the same adapter-vocabulary classifier.
              if (options?.compaction && !isTruncatedStopReason(event.stopReason)) {
                if (!isCleanCompactionStopReason(event.stopReason)) {
                  emitCompactionFailure(UNKNOWN_COMPACTION_STOP_ERROR, event.usage);
                  terminalEvent = true;
                  break;
                }
                // A successful empty envelope (`ocx1:`) replaces the entire prior conversation
                // with no handoff. Treat an empty visible final as an invalid upstream response;
                // internal thinking is intentionally not promoted into replacement history.
                if (!hasUsableCompactionSummary(compactionText)) {
                  emitCompactionFailure(EMPTY_COMPACTION_SUMMARY_ERROR, event.usage);
                  terminalEvent = true;
                  break;
                }
...
function buildResponseJSONWithBudget(
  ...
  let cleanDone = false;
  let compactionDoneSeen = false;
  let compactionDoneStopReason: string | undefined;
  ...
  for (const e of events) {
    // Match the streaming contract exactly: a compaction response contains only the
    // synthetic replacement item. Visible final text is retained as its payload; thinking,
    // tools, and other intermediate events are neither exposed nor treated as a summary.
    if (options?.compaction) {
      ...
      if (e.type !== "done" && e.type !== "incomplete" && e.type !== "error") {
        if (budget) releaseTranslatedEvent(e, budget);
        continue;
      }
    }
  ...
  const setCompactionFailure = (message: string): void => {
    if (compactionTextBytes > 0) {
      budget?.releaseRetained(compactionTextBytes, { kind: "retained_collectors" });
      compactionTextBytes = 0;
    }
  ...
  if (
    ...
    && !errorEvent
    && !incompleteEvent
    // A stream that stopped without any terminal did not complete either. Do not require
    // `sawTerminal` here: the first branch below must turn adapter EOF into a compaction-specific
    // failure instead of letting the generic buffered path report only `incomplete`.
    && !isTruncatedStopReason(rawStopReason)
  ) {
    if (!compactionDoneSeen) setCompactionFailure(MISSING_COMPACTION_TERMINAL_ERROR);
    else if (!isCleanCompactionStopReason(compactionDoneStopReason)) setCompactionFailure(UNKNOWN_COMPACTION_STOP_ERROR);
    else if (!hasUsableCompactionSummary(compactionText)) setCompactionFailure(EMPTY_COMPACTION_SUMMARY_ERROR);
  }
  if (
    options?.compaction
    && !errorEvent
    && !incompleteEvent
    && compactionDoneSeen
    && isCleanCompactionStopReason(compactionDoneStopReason)
