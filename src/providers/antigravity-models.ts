...
/** Current-generation suffix IDs still present in saved configs and older discovery payloads. */
const CURRENT_FLASH_SUFFIX_TIERS: Record<string, string> = {
...
// Gemini 3.1: effort → wire-model suffix. Gemini 3.7 and Claude models:
// effort → thinkingConfig.thinkingLevel against one wire id.
...
// Live wire suffix IDs may resolve to themselves. Retired Flash suffixes redirect to the
// current tiered wire while RETIRED_FLASH_TIERS preserves the level encoded by the saved id.
...
/**
 * Whether the given model ID is a suffix wire ID or compatibility alias instead of a
 * picker-visible base. Most such IDs own their effort on the wire; current and retired
 * Flash suffixes are handled first by resolveAntigravityEffortWireModel because CCA now
 * requires their encoded tier in thinkingConfig against one `-tiered` wire id.
 */
 ...
 * 1. Current or retired Flash suffix → current tiered wire ID + thinkingLevel.
 * 2. Other suffix wire ID or compat alias → resolve via `resolveAntigravityWireModelId`, no thinkingConfig.
...
export function resolveAntigravityEffortWireModel(
  ...
  // Rule 0: Flash suffix id — Google now accepts one `-tiered` wire id, so both
  // current-generation suffixes from older discovery payloads and retired saved ids
  // must carry their encoded level through thinkingLevel. This runs before the generic
  // suffix check, which would otherwise send a dead suffixed wire id and drop the tier.
  const currentFlashTier = Object.hasOwn(CURRENT_FLASH_SUFFIX_TIERS, modelId)
    ? CURRENT_FLASH_SUFFIX_TIERS[modelId]
    : undefined;
  const encodedFlashTier = currentFlashTier ?? retiredAntigravityFlashTier(modelId);
  if (encodedFlashTier) {
    return {
      wireModelId: GEMINI_FLASH_WIRE_ID,
      thinkingLevel: effort
        ? resolveAntigravityThinkingLevel(effort) ?? encodedFlashTier
        : encodedFlashTier,
    };
  }
  ...
  if (effortMap) {
    if (effort && Object.hasOwn(effortMap, effort)) {
...
export function canonicalAntigravityUsageModel(modelId: string): string {
  ...
  return Object.hasOwn(ANTIGRAVITY_USAGE_BASE_BY_ID, id)
    ? ANTIGRAVITY_USAGE_BASE_BY_ID[id]!
    : id;
}
