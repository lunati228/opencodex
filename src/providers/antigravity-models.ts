 ...
 * 1. Saved 3.7 Flash suffix → the 3.7 tiered wire ID + thinkingLevel.
...
export function resolveAntigravityEffortWireModel(
  ...
  // Rule 0a: saved 3.7 Flash suffix id. That generation accepts one `-tiered`
  // wire id, so old per-tier selectors must carry their encoded level through
  // thinkingLevel. This is deliberately 3.7-only: 3.8 tiers are live suffix
  // wire ids and are resolved by discovery/the static effort map below.
  ...
  if (currentFlashTier) {
    return {
      ...
      thinkingLevel: effort
        ? resolveAntigravityThinkingLevel(effort) ?? currentFlashTier
        : currentFlashTier,
    };
  }
  ...
  // Rule 0b: retired Flash id — Google has taken the wire id offline, so route to the 3.7
