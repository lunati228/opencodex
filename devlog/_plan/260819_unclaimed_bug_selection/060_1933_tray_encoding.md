...
this for `schtasks` — that was #1573, and it ships with a `<private-user-home>`
...
`<private-user-home>\.opencodex\opencodex-tray.vbs` through the tray registry
reader and assert
...
profile path was never posted, and `<private-user-home>` is reconstructed
