...
## The harness, exactly
    ...
    ssh <remote-ci-host> 'git clone --branch dev https://github.com/lidge-jun/opencodex ~/ocx-verify/repo'
    ssh <remote-ci-host> 'cd ~/ocx-verify/repo && bun install'

    # per verification round
    git push --no-verify -u origin codex/base-prompt-variants
    ssh <remote-ci-host> 'cd ~/ocx-verify/repo && git fetch origin codex/base-prompt-variants \
      && git switch --detach FETCH_HEAD && bun install'
    ssh <remote-ci-host> 'export PATH=$HOME/bin:$PATH
    ...
    ssh <remote-ci-host> 'export PATH=$HOME/bin:$PATH; ocx-run status'
    ssh <remote-ci-host> 'export PATH=$HOME/bin:$PATH; ocx-run tail gate 60'
