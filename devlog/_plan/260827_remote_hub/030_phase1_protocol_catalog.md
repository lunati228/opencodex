...
```bash
ssh host 'cd ~/Developer/opencodex && bun run typecheck && bun test tests/config.test.ts tests/server-live.test.ts tests/proxy-liveness.test.ts tests/api-catalog-route.test.ts tests/server-auth.test.ts tests/api-key-attribution.test.ts tests/core-lab-boundary.test.ts'
```

Review-ready shared-server gate:

```bash
ssh host 'cd ~/Developer/opencodex && bun run test && bun run privacy:scan'
```
