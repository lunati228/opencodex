...
(`ssh remote` via `ocx-run` at the exact release sha) and to let the workflow gates be the
...
The suite still runs — on `ssh remote` at the exact release sha, as every phase of the
hardening unit did.
