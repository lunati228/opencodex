...
The suite runs on `ssh remote` via `ocx-run` at that exact sha, as every phase of the
...
| Gate | Evidence |
| --- | --- |
| full suite at the promoted tree | `pvsuite` on `ssh remote`, 15334 pass / 0 fail, rc=0, at `62dfc6c54` |
