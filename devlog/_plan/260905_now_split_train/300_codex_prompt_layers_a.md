...
```sh
ssh ci-host "bash -lc 'set -o pipefail; cd ~/ocx-ci/opencodex && git fetch origin codex/split-codex-prompt-layers-a && git checkout -q FETCH_HEAD && git rev-parse HEAD && bun install --frozen-lockfile >/dev/null && bun run test 2>&1 | tail -15'"
```
