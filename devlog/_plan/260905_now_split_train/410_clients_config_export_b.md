...
- Verifier: `002_layer_map.md` **Per-layer gate**, instantiated below; every layer must pass independently at its actual tip. Full suite on `ssh ci-host` only, never locally.
...
ssh ci-host 'cd ~/ocx-ci/opencodex && git fetch origin codex/split-clients-config-export-b && git checkout -q FETCH_HEAD && bun install --frozen-lockfile >/dev/null && bun run test'
```
