...
ssh ci-host 'set -o pipefail; cd ~/ocx-ci/opencodex && git fetch origin codex/split-server-responses-collaboration && git checkout -q FETCH_HEAD && bun install --frozen-lockfile >/dev/null && bun run test 2>&1 | tail -15'
```
