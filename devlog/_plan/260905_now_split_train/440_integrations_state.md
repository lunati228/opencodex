...
ssh ci-host 'cd ~/ocx-ci/opencodex && git fetch origin codex/split-integrations-state && git checkout -q FETCH_HEAD && bun install --frozen-lockfile >/dev/null && bun run test'
```
