...
ssh ci-host 'cd ~/ocx-ci/opencodex && git fetch origin codex/split-pages-Storage-b && git checkout -q FETCH_HEAD && bun install --frozen-lockfile && bun run test && cd gui && bun test tests && bun run lint && bun run build'
```
