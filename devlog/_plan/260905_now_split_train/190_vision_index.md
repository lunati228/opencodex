...
```sh
ssh ci-host 'bash -lc '\''set -o pipefail; cd ~/ocx-ci/opencodex && git fetch origin codex/split-vision-index && git checkout -q FETCH_HEAD && git rev-parse HEAD && bun install --frozen-lockfile >/dev/null && bun run test 2>&1 | tee /tmp/ocx-S06-L1-tests.log | tail -15'\'''
```
