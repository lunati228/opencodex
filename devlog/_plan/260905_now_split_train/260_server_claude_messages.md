...
```sh
ssh ci-host 'set -e; cd ~/ocx-ci/opencodex; git fetch origin codex/split-server-claude-messages; git checkout -q FETCH_HEAD; git rev-parse HEAD; bun install --frozen-lockfile >/dev/null; bun run test'
```
