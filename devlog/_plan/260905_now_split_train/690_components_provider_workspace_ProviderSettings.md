...
ssh ci-host 'cd ~/ocx-ci/opencodex && git fetch origin codex/split-components-provider-workspace-ProviderSettings && git checkout -q FETCH_HEAD && git rev-parse HEAD && bun install --frozen-lockfile && bun run test'
# Full GUI PR-ready suite also remote, never substitute it for the root suite:
ssh ci-host 'cd ~/ocx-ci/opencodex/gui && bun install --frozen-lockfile && bun test tests'
```
