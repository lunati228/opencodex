...
ssh ci-host 'cd ~/ocx-ci/opencodex && git fetch origin codex/split-components-MemoryObservabilityCard && git checkout -q FETCH_HEAD && git rev-parse HEAD && bun install --frozen-lockfile && bun run test'
