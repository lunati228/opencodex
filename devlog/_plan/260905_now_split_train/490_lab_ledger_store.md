...
ssh ci-host 'bash -o pipefail -c "cd ~/ocx-ci/opencodex && git fetch origin codex/split-lab-ledger-store && git checkout -q FETCH_HEAD && git rev-parse HEAD && bun install --frozen-lockfile && bun run test 2>&1 | tail -15"'
```
