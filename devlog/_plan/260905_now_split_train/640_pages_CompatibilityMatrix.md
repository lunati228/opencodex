...
```sh
ssh ci-host 'cd ~/ocx-ci/opencodex && git fetch origin codex/split-pages-CompatibilityMatrix && git checkout -q FETCH_HEAD && bun install --frozen-lockfile && bun run test && (cd gui && bun install --frozen-lockfile && bun test tests)'
```
