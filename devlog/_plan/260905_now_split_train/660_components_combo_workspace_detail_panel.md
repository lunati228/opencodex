...
```sh
ssh ci-host 'cd ~/ocx-ci/opencodex && git fetch origin codex/split-components-combo-workspace-detail-panel && git checkout -q FETCH_HEAD && bun install --frozen-lockfile && bun run test && (cd gui && bun install --frozen-lockfile && bun test tests)'
```
