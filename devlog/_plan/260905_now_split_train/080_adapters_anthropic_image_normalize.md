...
```sh
ssh ci-host 'cd ~/ocx-ci/opencodex && git fetch origin codex/split-adapters-anthropic-image-normalize && git checkout -q FETCH_HEAD && bun install --frozen-lockfile >/dev/null && bun run test 2>&1 | tail -15'
```
