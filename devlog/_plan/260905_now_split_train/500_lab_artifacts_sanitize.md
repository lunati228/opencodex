...
- Verifier: `002_layer_map.md` **Per-layer gate**, instantiated below; full tests only on `ssh ci-host`, never locally.
...
ssh ci-host 'bash -o pipefail -c "cd ~/ocx-ci/opencodex && git fetch origin codex/split-lab-artifacts-sanitize && git checkout -q FETCH_HEAD && git rev-parse HEAD && bun install --frozen-lockfile && bun run test 2>&1 | tail -15"'
```
