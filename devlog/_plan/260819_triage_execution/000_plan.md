...
current dev in a <private-ssh-host> worktree and run the focused suites the diff touches
...
- Merge method: squash when the branch has fixup/noise commits, merge
  otherwise.
- Suites: <private-ssh-host> dispatch without long blocking waits; decisive gate is the
  Cross-platform CI push run on the merged SHA.

## Known risks

- Seven merge candidates lack head CI (see pre-merge validation rule above);
  scratch-merge <private-ssh-host> validation is mandatory for them.
