...
- Worktree: <private-user-home>/.codex/worktrees/f9b0/opencodex (detached at origin/dev tip
...
- Full suite runs on ssh <private-ssh-host> only (bun test --isolate tests); local gates are
...
- Remote: ssh <private-ssh-host> 'cd ~/Developer/opencodex && git fetch && git checkout <merged dev>
  && bun test --isolate tests' before final DONE claim.

## Expected terminal outcomes

DONE = all four PRs merged into dev, <private-ssh-host> isolated suite green, browser repro
