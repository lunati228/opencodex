...
- Per-PR scratch worktree on <private-ssh-host> or local: bun test <named suites> + tsc.
- #1876 additionally requires the platform-windows workflow_dispatch run green on
  the exact landed SHA (<private-ssh-host> is not a Windows leg and does not discharge it).
- gh pr checks after any push; <private-ssh-host> full suite before wp11 closeout (not per-merge).
