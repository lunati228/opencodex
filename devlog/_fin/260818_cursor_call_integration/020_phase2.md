# 020 — WP3: full remote verification on ssh <private-ssh-host>
...
Standing user contract: the authoritative suite runs on `ssh <private-ssh-host>`, never locally.

<private-ssh-host>: `<private-user-home>/Developer/opencodex`, bun 1.3.14, 16 cores.
...
would silently discard any tracked uncommitted work. `git worktree list` on <private-ssh-host>
...
provably the SHA it tested (audit `r13`):
    ...
    ssh <private-ssh-host> "cd ~/Developer/opencodex && git fetch origin cursor-call dev && git worktree add /tmp/ocx-cc-${VERIFIED_TIP:0:9} $VERIFIED_TIP"
    ssh <private-ssh-host> "cd /tmp/ocx-cc-${VERIFIED_TIP:0:9} && test \"\$(git rev-parse HEAD)\" = \"$VERIFIED_TIP\" && bun install --frozen-lockfile"
...
```
ssh <private-ssh-host> 'cd ~/Developer/opencodex && git rev-parse origin/cursor-call'   # == local rebase tip
```
...
`VERIFIED_TIP`:
    ...
    do
      ssh <private-ssh-host> "cd $CC_WT && test \"\$(git rev-parse HEAD)\" = \"$VERIFIED_TIP\" && $GATE"
    done
...
<private-ssh-host> is Linux. Repository CI covers Linux, Windows, and macOS. This campaign's
...
3. `020` this half: one worktree per layer, pinned to that layer's head. The test
   ...
   `<that layer's files>` is a zsh parse error, not an instruction:
       ...
       run_layer() {
         ...
         ssh <private-ssh-host> "cd ~/Developer/opencodex && git fetch origin && git worktree add $WT $SHA"
         ssh <private-ssh-host> "cd $WT && test \"\$(git rev-parse HEAD)\" = \"$SHA\" && bun install --frozen-lockfile"
         ssh <private-ssh-host> "cd $WT && bun x tsc --noEmit"
         ssh <private-ssh-host> "cd $WT && bun test $TESTS"
       }
