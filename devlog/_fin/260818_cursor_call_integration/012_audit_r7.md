...
`020` has <private-ssh-host> fetch `origin/cursor-call` and create a worktree at the new SHA, and
`010` said the audit happens "before pushing" — but no push command existed anywhere
between them. <private-ssh-host> would have fetched the PRE-rebase branch and then either failed
...
someone else's push is not. `020` also re-confirms the SHA on <private-ssh-host> before installing.
