...
read. Build the worktree at it and assert what landed there:
    ...
    ssh <private-ssh-host> "cd ~/Developer/opencodex && git fetch origin dev && git worktree add $DEVDIR $MERGED_DEV"
    ssh <private-ssh-host> "cd $DEVDIR && test \"\$(git rev-parse HEAD)\" = \"$MERGED_DEV\" && bun install --frozen-lockfile"
    ssh <private-ssh-host> "cd $DEVDIR && bun x tsc --noEmit"
    ssh <private-ssh-host> "cd $DEVDIR && bun run privacy:scan"
    ssh <private-ssh-host> "cd $DEVDIR && bun run audit:high"
    ssh <private-ssh-host> "cd $DEVDIR && bun run build:gui"
    ssh <private-ssh-host> "cd $DEVDIR && bun test --isolate tests"
