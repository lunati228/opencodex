...
7. **Push the rebased branch (audit `r7`).** WP3 verifies on <private-ssh-host> by fetching
   `origin/cursor-call`, so an unpushed rebase means <private-ssh-host> either fails on an unknown
...
Authoritative verification is WP3 on `ssh <private-ssh-host>`. Expected: typecheck exit 0; the
