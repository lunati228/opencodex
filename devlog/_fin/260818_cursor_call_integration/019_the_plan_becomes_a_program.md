...
| `verify` | `020` | `VERIFIED_TIP`, <private-ssh-host> worktree at that SHA, five gates each re-asserting HEAD |
...
| `release_gates` | `050` | `MERGED_DEV` worktree on <private-ssh-host>, five gates |
...
- **`REMOTE_HOME=~/Developer/opencodex` expanded LOCALLY.** zsh resolved `~` to
