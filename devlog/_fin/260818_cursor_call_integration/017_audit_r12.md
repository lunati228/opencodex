...
| Round | The gap |
|-------|---------|
| r7 | No push between the rebase (WP2) and remote verification (WP3) — <private-ssh-host> would fetch the pre-rebase branch |
| r8 | That push ran BEFORE WP2b changed code, so <private-ssh-host> would bless a tree without WP2b |
...
`020` created the <private-ssh-host> worktree at an unnamed `<SHA>`, ran every gate, and only then
...
3. one <private-ssh-host> worktree per layer head, pinned and asserted, running that layer's
