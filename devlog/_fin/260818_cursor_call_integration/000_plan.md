...
- Loop archetype: verifier-defined (typecheck + full suite on <private-ssh-host> decide done).
...
- Tool/credential scope: local git, `ssh <private-ssh-host>` for verification, `gh`/GitHub app
...
- Bounds: no stated token budget. Wall-clock dominated by the <private-ssh-host> suite (~8 min).
...
| wp3-remote-verify | `020` | typecheck + full suite + privacy:scan + audit:high on <private-ssh-host> | wp2b |
...
- `c5-remote-green` — typecheck + full suite green on <private-ssh-host> at the SHA.
