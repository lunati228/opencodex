...
remote_worktree () {
  local wt="$1" sha="$2"
  ssh <private-ssh-host> "cd $REMOTE_HOME && { git worktree list --porcelain | grep -qx 'worktree $wt' || git worktree add $wt $sha; }" \
    || die "could not create $wt at $sha on <private-ssh-host>"
  ssh <private-ssh-host> "cd $wt && test \"\$(git rev-parse HEAD)\" = \"$sha\" && test -z \"\$(git status --porcelain)\"" \
    || die "$wt is not a clean checkout of $sha"
}

remote_worktree_remove () {
  ssh <private-ssh-host> "cd $REMOTE_HOME && git worktree remove --force $1" \
...
step_pin () {
  ...
  if [[ "${1:-}" == "--repin" && -f "$STATE" ]]; then
    ...
    # them on <private-ssh-host> with no handle left to clean them up (audit r16).
...
# <private-user-home>/... does not exist on <private-ssh-host>. The remote shell expands this (audit r15).
REMOTE_HOME='$HOME/Developer/opencodex'
...
step_verify () {
  ...
  ssh <private-ssh-host> "cd $REMOTE_HOME && git fetch origin cursor-call dev" || die "<private-ssh-host> fetch failed"
  remote_worktree "$wt" "$tip"
  save CC_WORKTREE "$wt"
  ssh <private-ssh-host> "cd $wt && bun install --frozen-lockfile"
  local gate
  for gate in "bun x tsc --noEmit" "bun run privacy:scan" "bun run audit:high" "bun run build:gui" "bun test --isolate tests"; do
    log "gate: $gate"
    local out
    out="$(ssh <private-ssh-host> "cd $wt && test \"\$(git rev-parse HEAD)\" = \"$tip\" && $gate" 2>&1)" \
...
verify_layer () {
  ...
  ssh <private-ssh-host> "cd $REMOTE_HOME && git fetch origin cursor-call-wire cursor-call-cancel" || die "<private-ssh-host> fetch failed"
  # Recorded before the gates so a failure leaves a cleanup handle (audit r16).
  save LAYER_WORKTREE "$wt"
  remote_worktree "$wt" "$sha"
  ssh <private-ssh-host> "cd $wt && bun install --frozen-lockfile"
  ssh <private-ssh-host> "cd $wt && test \"\$(git rev-parse HEAD)\" = \"$sha\" && bun x tsc --noEmit" \
    || die "typecheck failed at layer $sha"
  ssh <private-ssh-host> "cd $wt && test \"\$(git rev-parse HEAD)\" = \"$sha\" && bun test $tests" \
...
step_release_gates () {
  ...
  ssh <private-ssh-host> "cd $REMOTE_HOME && git fetch origin dev" || die "<private-ssh-host> fetch failed"
  ...
  for gate in "bun x tsc --noEmit" "bun run privacy:scan" "bun run audit:high" "bun run build:gui" "bun test --isolate tests"; do
    log "dev gate: $gate"
    local out
    out="$(ssh <private-ssh-host> "cd $wt && test \"\$(git rev-parse HEAD)\" = \"$MERGED_DEV\" && $gate" 2>&1)" \
