...
```bash
ssh <remote-ci-host> 'cd <checkout> && git fetch origin && git checkout <branch> && bun install --frozen-lockfile'
ssh <remote-ci-host> 'cd <checkout> && bun x tsc --noEmit'
ssh <remote-ci-host> 'cd <checkout> && bun test tests/cursor-tool-call-replay.test.ts tests/cursor-blob.test.ts \
