...
- `bun x tsc --noEmit` and `bun run privacy:scan`; full suite on `ssh <remote-ci-host>`, never locally.
...
`ssh <remote-ci-host>` was re-run against the merge commit itself and reported exit 0 with 16359 pass / 0 fail /
