...
one dev CI run, one <private-ssh-host> validation before the push.
...
ssh <private-ssh-host>: clone/fetch, checkout int branch, bun install, bun run typecheck +
bun run test (+ privacy:scan). Suite runs ONLY on <private-ssh-host> per owner directive.
...
- <private-ssh-host> validation: worktree /tmp/ocx-triage-260815-int on ba20ce17f;
