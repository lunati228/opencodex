...
- Per wave: `ssh <private-ssh-host>` `bun test --isolate tests`, compared against a `dev` run at the same commit. A failure present on both sides is not a regression; a failure present only on the branch is.
