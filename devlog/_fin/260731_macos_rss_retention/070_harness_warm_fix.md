...
- **Post-patch remote smoke calibration (audit round 1 blocker 6):** after the
  patch lands locally, sync it to `<remote-macos-host>` (`~/rss-measure/opencodex`) and
...
- Remote smoke calibration on `<remote-macos-host>` (Darwin arm64, patched file synced
