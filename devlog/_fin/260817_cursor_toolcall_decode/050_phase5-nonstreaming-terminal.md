...
- runs the **full** suite on `ssh <private-ssh-host>`, not the cursor subset. A baseline full
...
All five pass, `bun run typecheck` clean, and the **full** suite on `ssh <private-ssh-host>`
...
Final verification on `ssh <private-ssh-host>` at `1651002c59`: typecheck clean,
