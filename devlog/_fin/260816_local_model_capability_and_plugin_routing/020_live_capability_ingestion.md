...
2. Mapping the `multimodal` capability token to image input. Audit round 4
   ...
   Reviewer proof:
       ...
       catalogHintsFromModelsApiItem("<private-ssh-host>", {
...
so proved nothing.
    ...
    test("absorbs meta.n_ctx from the verbatim llama.cpp data[] item", () => {
      ...
      const hints = catalogHintsFromModelsApiItem("<private-ssh-host>", {
    ...
    test("the dual-envelope body still yields no image evidence (documents the gap)", () => {
      ...
      const hints = catalogHintsFromModelsApiItem("<private-ssh-host>", extracted.items[0]);
      expect(hints.contextWindow).toBe(262144);
...
5. The targeted suites pass on <private-ssh-host> at the pushed head, and the full suite
...
| `export PATH` + `command -v bun` | `command -v bun` empty over non-interactive ssh while `<private-user-home>/.bun/bin/bun` exists |
