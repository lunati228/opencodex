...
amends this doc if it differs.
    ...
    test("carries asserted context window and image modality from a catalog-only model", () => {
      ...
      const evidence = candidateCapabilityEvidence(config, "<private-ssh-host>", "qwen3.8-27b-nvfp4");
...
6. Remote exact-head suite green on <private-ssh-host> (command in the section below).
