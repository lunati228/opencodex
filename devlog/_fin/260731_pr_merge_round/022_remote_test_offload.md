...
### 2. 원격 오프로드 — `<remote-macos-host>`

무거운 스위트는 로컬에서 돌릴 이유가 없다. 사용자가 병렬로 작업 중이면
서로를 방해할 뿐이다.

```
Host <remote-macos-host>
    HostName ssh-macmini.<private-remote-account>.com
    User <remote-user>
...
scp /tmp/x.bundle <remote-macos-host>:/tmp/
ssh <remote-macos-host> 'cd ~/ci/opencodex && git fetch /tmp/x.bundle <branch>:<branch> && git checkout <branch>'
```

실행:

```bash
ssh <remote-macos-host> 'export PATH=$HOME/.bun/bin:$PATH; cd ~/ci/opencodex && \
