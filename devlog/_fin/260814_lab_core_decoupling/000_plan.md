...
| 5 | [`050`](./050_governance_and_release.md) | CODEOWNERS/branch protection, verification on `<private-ssh-host>`, PR, release | 1–4 |
...
5. Full suite green on the remote Linux runner `<private-ssh-host>`.
6. Core-file changes require owner approval going forward.

## Verification

Local: focused `bun test` per phase, then `bun x tsc --noEmit`.
Remote: full suite on `<private-ssh-host>` (`~/.bun/bin/bun`, Bun 1.3.14, Ubuntu). The local pre-push
hook (`.git/hooks/pre-push` → `bun run prepush`) is bypassed with `--no-verify` because
the authoritative suite run happens on `<private-ssh-host>`.
