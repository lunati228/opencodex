...
## Remote Linux runner (`<private-ssh-host>`, Ubuntu, 16 cores, Bun 1.3.14)
...
Remote runner `<private-ssh-host>` (Ubuntu, Bun 1.3.14), branch at `41061b241`, GUI deps installed
as CI does:
...
suite evidence was produced on `<private-ssh-host>` and is now confirmed by CI itself.
...
This independently confirms the `<private-ssh-host>` result and closes out the earlier unsharded
