...
Authoritative (Linux, matching CI): remote runner `<private-ssh-host>` — Ubuntu, Bun 1.3.14 at
`~/.bun/bin/bun`.

```bash
rsync the working tree to a scratch dir on <private-ssh-host>
~/.bun/bin/bun install
...
the authoritative suite run happens on `<private-ssh-host>`. The bypass skips a duplicate local run, not
verification: the `<private-ssh-host>` evidence is recorded before the push.
...
`<private-ssh-host>` evidence, and links this devlog unit. No `gui` mention, so no screenshot gate.
...
- `<private-ssh-host>` typecheck and suite evidence recorded.
