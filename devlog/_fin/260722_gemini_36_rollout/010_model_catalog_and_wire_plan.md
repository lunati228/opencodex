...
- Commit the completed branch, then integrate it into `<private-user-home>/Developer/new/700_projects/opencodex` on local `dev` with a normal `git merge --no-ff gemini-3.6`. Preserve the unrelated dirty issue-sweep devlog file exactly by comparing its pre/post content hash; verify the merge commit contains the Gemini tip and rerun the focused smoke test from `dev`. Do not push.
- After all merged-`dev` checks pass, run `git worktree remove <private-user-home>/.codex/worktrees/2d67/opencodex` from the main repository. Do not delete the merged branch ref and do not force cleanup over a dirty Gemini worktree.
