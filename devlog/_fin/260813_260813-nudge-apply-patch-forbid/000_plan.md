...
- Budget: one work-phase; full suite runs on `ssh <private-ssh-host>`; push to `dev` with
...
- c3: repository suite on `ssh <private-ssh-host>` green, or zero NEW failures versus baseline.
