...
# ssh <private-ssh-host>, full suite (per user instruction)
ssh <private-ssh-host> 'cd ~/Developer/opencodex && git fetch origin && git checkout dev && git pull && bun install && bun run test'
```
