...
- Verifier: `bun run typecheck`, focused `bun test` per fix, full
  `bun run test` on `ssh <private-ssh-host>` (Linux baseline recorded below),
...
- Worktree: `<private-user-home>/.codex/worktrees/250c/opencodex`, branch
  ...
  `dev`).
- <private-ssh-host> Linux baseline (`bun run test` on dev HEAD, 2026-08-05): the first
  ...
  EXIT=0** (`/tmp/ocx-baseline-test2.log` on <private-ssh-host>). Campaign deltas are
