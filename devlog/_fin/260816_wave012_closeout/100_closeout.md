...
- Per wave: `ssh <private-ssh-host>` full suite in `<private-user-home>/Developer/opencodex` with `bun test --isolate tests`, reporting pass/fail counts. Raw `bun test` in that checkout bleeds environment state across files and is not acceptable evidence.
...
- Contributor-branch CI is `action_required` and cannot be treated as green. Authorize the workflow runs and get the exact-head matrix **before** merging; `MAINTAINERS.md` requires maintainer approval plus successful CI, and post-merge testing is not a substitute. Windows-specific changes (`#1805`, `#1806`) specifically require the native Windows shards — the Linux `ssh <private-ssh-host>` suite cannot stand in for them.
