...
Both PRs change Windows-specific behavior, so a Linux/macOS run — including the `ssh <private-ssh-host>` suite — is not equivalent evidence. Native Windows CI must run at the exact head.
