...
Full Linux suite on `ssh <private-ssh-host>` at the final head: **12651 pass / 15 skip / 16 fail**. Every failure also fails on `dev` at the same commit — GUI lint/doctor spawner harnesses, multi-process lock contention, a typecheck-contract case, and one `bun`-not-on-PATH harness case. No routing, adapter, config or server test fails.
