...
Checkout: `<home>/.codex/worktrees/4b3a/opencodex`
...
`cxc map tests` is present (`<home>/.nvm/versions/node/v24.17.0/bin/cxc`) but emits ranked function maps for individual files, not a directory inventory. Counts below come from `find` + a Python walk of the live tree.
