...
Linux cross-check on `<private-ssh-host>` — procfs enumeration
...
```bash
ssh <private-ssh-host> 'cd ~/Developer/opencodex && ~/.bun/bin/bun run typecheck \
...
| `ssh <private-ssh-host> ... bun test ...` | yes — same files on procfs |
